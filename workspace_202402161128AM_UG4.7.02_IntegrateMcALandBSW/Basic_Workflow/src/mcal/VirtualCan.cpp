/*******************************************************************************
 * Autogenerated by MCALgen for MCAL V12.2.0.35287: VirtualCan.cpp
 *******************************************************************************/
...
#define CAN_NO_RX_INDEX             ((unsigned) -1)
#define MAX_TIMESTAMP_NANOSECONDS   (999999999U) // Nanoseconds: 0 .. 999.999.999 ns uint32 so must start at 0
#define MAX_TIMESTAMP_SECONDS       (4294967295U) // Seconds: 0 .. 4.294.967.295 s (circa 136 years) uint32 so must start at 0
...
// ---------------------------------------------------------------------------
class Can_Virtual_TxObject { // TODO: If this is identical across all hardware provider implementations, consider using
// the provider-agnostic Can_Hwp_TxObject in `Can_HwProviderTypes.h` (similar to the receive objects). Only once the
// filtering refactor has been completed though.
public:
  Can_HwHandleType m_Handle;
  Can_Hwp_Frame    m_Frame;
  uint8 m_FdPaddingValue;
  uint8 m_ObjectPayloadLength;
  Can_TimeStampType m_TimeStamp;
  volatile bool     m_TimeStampEnable;
  ...
  Can_Virtual_TxObject(void)
    : m_Handle(0U),
      ...
      m_Cancel(false) {
        m_FdPaddingValue = 0;
        m_ObjectPayloadLength = 8;
        m_TimeStamp.seconds = 0;
        m_TimeStamp.nanoseconds = 0;
        m_TimeStampEnable = false;
...
public:
  ...
  Can_Hwp_RxHwObject*   m_RxObjects;
  unsigned              m_Baudrate;
  unsigned              m_FdBaudrate;
  bool                  m_FdTxBitRateSwitch;
  volatile uint8         m_RxErrorCounter;
  volatile uint8         m_TxErrorCounter;
  volatile unsigned      m_Error;
  volatile Can_TimeStampType m_TimeStamp;
  Can_TimeStampType* m_RxObjectsFakeTimestamp;
  // These are used to communicate with the worker thread - hence they are
  // volatile and should be read/written with a single instruction.
  ...
  Can_Virtual_Controller(void)
    : m_CtrlName(""),
      ...
      m_TxErrorCounter(0U),
      m_Error(0U) ,
      m_RxObjectsFakeTimestamp(NULL_CPP_PTR),
      // [$CANHWP 4]
  ...
  ~Can_Virtual_Controller(void) {
    ...
    delete [] m_RxObjects;
    delete [] m_RxObjectsFakeTimestamp;
  }
...
//    }
  ...
  void ConfigureController(uint32 hwId, const Can_Hwp_ChannelConfig* config) {
    ...
    for (unsigned i = 0U; i < ctrl->m_NumTxObjects; i++) {
      ...
      ctrl->m_TxObjects[i].m_ObjectPayloadLength = config->txHwObjects[i].objectPayloadLength;
      ctrl->m_TxObjects[i].m_TimeStamp.seconds = 0;
      ctrl->m_TxObjects[i].m_TimeStamp.nanoseconds = 0;
    ...
    ctrl->m_RxObjects = new Can_Hwp_RxHwObject[ctrl->m_NumRxObjects];
    ctrl->m_RxObjectsFakeTimestamp = new Can_TimeStampType[ctrl->m_NumRxObjects];

    for (unsigned i = 0U; i < ctrl->m_NumRxObjects; i++) {
      Can_Hwp_RxHwObject* rxObj = &ctrl->m_RxObjects[i];
      const Can_Hwp_RxHwObject* rxObjConfig = &config->rxHwObjects[i];
      Can_LoadRxObjFromConfig(rxObj, rxObjConfig);
      ctrl->m_RxObjectsFakeTimestamp[i].seconds =0;
      ctrl->m_RxObjectsFakeTimestamp[i].nanoseconds =0;
    }
  ...
  Can_ReturnType Write(uint32 hwId, Can_HwHandleType handle, Can_Hwp_Frame* frame) {
    ...
    if (frame->length > txObj->m_ObjectPayloadLength) {  /* [$CAN21 9512] */
      mcal_printf("VirtualCan", "Write:- Transmit Frame dropped frame length %u > object payload length %u\n", frame->length, txObj->m_ObjectPayloadLength);
    ...
    if (ctrl->m_CurrentState == CAN_HWP_EVENT_RUNNING) {
      ...
      if (txObj->m_InUse) {
        ...
        txObj->m_Cancel = false;
                  if(CAN_ID_FD_BIT_SET(frame->id)) {  /* [$CAN 1163] [$CAN21 502] [$CAN 1229] [$CAN 1230] [$CAN 1233] */
            ...
            for(int i = frameLength; i < frame->length; i++) { /*  [$CAN 1203] [$CAN21 9485] [$CAN 1234] */
  ...
  // ---------------------------------------------------------------
  void ForceError(uint32 hwId, uint8 error) {
    if (!m_Initialised || hwId >= m_NumControllers) {
      return;
    }
    Can_Virtual_Controller* ctrl = &m_Controllers[hwId];
  ...
  // ---------------------------------------------------------------
  Std_ReturnType GetControllerTxErrorCounter(uint8 hwId, uint8* TxErrorCounterPtr) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    Can_Virtual_Controller* ctrl = &m_Controllers[hwId];
    ...
    *TxErrorCounterPtr = ctrl->m_TxErrorCounter;
    return E_OK;
  }

  // ---------------------------------------------------------------
  void InjectControllerTxErrorCount(uint8 hwId, uint8 TxErrorCount) {
    if (!m_Initialised && !m_InitFailed) {
      return;
    }
    Can_Virtual_Controller* ctrl = &m_Controllers[hwId];
    ...
    ctrl->m_TxErrorCounter = TxErrorCount;
  }

  // ---------------------------------------------------------------
  Std_ReturnType GetControllerRxErrorCounter(uint8 hwId, uint8* RxErrorCounterPtr) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    Can_Virtual_Controller* ctrl = &m_Controllers[hwId];
    ...
    *RxErrorCounterPtr = ctrl->m_RxErrorCounter;
    return E_OK;
  }

  // ---------------------------------------------------------------
  void InjectControllerRxErrorCount(uint8 hwId, uint8 RxErrorCount) {
    if (!m_Initialised && !m_InitFailed) {
      return;
    }
    Can_Virtual_Controller* ctrl = &m_Controllers[hwId];
    ...
    ctrl->m_RxErrorCounter = RxErrorCount;
  }

  // ---------------------------------------------------------------
  Std_ReturnType CurrentTime(uint8 hwId, Can_TimeStampType* timeStampPtr, boolean fromAction) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    Can_Virtual_Controller* ctrl = &m_Controllers[hwId];
    ...
    timeStampPtr->nanoseconds = ctrl->m_TimeStamp.nanoseconds;
    timeStampPtr->seconds =  ctrl->m_TimeStamp.seconds;
    return E_OK;
  }

  // ---------------------------------------------------------------
  Std_ReturnType InjectCurrentTime(uint8 hwId, Can_TimeStampType timeStamp) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    Can_Virtual_Controller* ctrl = &m_Controllers[hwId];
    ...
    ctrl->m_TimeStamp.nanoseconds = timeStamp.nanoseconds;
    ctrl->m_TimeStamp.seconds = timeStamp.seconds;
    return E_OK;
  }

  // ---------------------------------------------------------------
  Std_ReturnType EnableEgressTimeStamp(uint8 hwId, Can_HwHandleType Hth) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    ...
    if (Hth < ctrl->m_LowestTxHandle || Hth > ctrl->m_HighestTxHandle) {
      return E_NOT_OK;
    }

    unsigned txIndex = ctrl->m_TxObjectsMap[Hth - ctrl->m_LowestTxHandle];
    if (txIndex == CAN_VIRTUAL_NO_TX_INDEX)  {
      return E_NOT_OK;
    }

    Can_Virtual_TxObject* txObj = &ctrl->m_TxObjects[txIndex];
    txObj->m_TimeStampEnable = true;

    return E_OK;
  }

  // ---------------------------------------------------------------
  Std_ReturnType InjectEnableEgressTimeStamp(uint8 hwId, Can_HwHandleType Hth, boolean enable) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    ...
    Can_Virtual_TxObject* txObj = &ctrl->m_TxObjects[txIndex];
    txObj->m_TimeStampEnable = enable;

    return E_OK;
  }

  // ---------------------------------------------------------------
  Std_ReturnType GetEgressTimeStamp(uint8 hwId, PduIdType TxPduId, Can_HwHandleType Hth, Can_TimeStampType* timeStampPtr) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    ...
    if (!txObj->m_TimeStampEnable) {
      mcal_printf("VirtualCan", "GetEgressTimeStamp:- Egress Timestamp Not enabled Hth: %u\n", Hth);
    }
    timeStampPtr->nanoseconds = txObj->m_TimeStamp.nanoseconds;
    timeStampPtr->seconds = txObj->m_TimeStamp.seconds;

    return E_OK;
  }

  // ---------------------------------------------------------------
  Std_ReturnType InjectEgressTimeStamp(uint8 hwId, PduIdType TxPduId, Can_HwHandleType Hth, Can_TimeStampType timeStamp) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    ...
    if (!txObj->m_TimeStampEnable) {
      mcal_error("VirtualCan", "InjectEgressTimeStamp:- Egress Timestamp not enabled\n");
      return E_NOT_OK;
    }

    if ((timeStamp.nanoseconds > MAX_TIMESTAMP_NANOSECONDS) || (timeStamp.seconds > MAX_TIMESTAMP_SECONDS)) {
      return E_NOT_OK;
    }

    txObj->m_TimeStamp.seconds = timeStamp.seconds;
    txObj->m_TimeStamp.nanoseconds = timeStamp.nanoseconds;

    return E_OK;
  }

  // ---------------------------------------------------------------
  Std_ReturnType GetIngressTimeStamp(uint8 hwId, Can_HwHandleType Hrh, Can_TimeStampType* timeStampPtr ) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    ...
    unsigned rxIndex = CAN_NO_RX_INDEX;
    for (unsigned j = 0U; j < ctrl->m_NumRxObjects; j++) {
        const Can_Hwp_RxHwObject& rxObj = ctrl->m_RxObjects[j];
        if (Hrh == rxObj.handle) {
          rxIndex = j;
          break;
        }
    ...
    if (rxIndex == CAN_NO_RX_INDEX)  {
      return E_NOT_OK;
    }

    timeStampPtr->nanoseconds = ctrl->m_RxObjectsFakeTimestamp[rxIndex].nanoseconds;
    timeStampPtr->seconds = ctrl->m_RxObjectsFakeTimestamp[rxIndex].seconds;

    return E_OK;
  }

  // ---------------------------------------------------------------
  Std_ReturnType InjectIngressTimeStamp(uint8 hwId, Can_HwHandleType Hrh, Can_TimeStampType timeStamp ) {
    if (!m_Initialised && !m_InitFailed) {
      return E_NOT_OK;
    }
    ...
    ctrl->m_RxObjectsFakeTimestamp[rxIndex].seconds = timeStamp.seconds;
    ctrl->m_RxObjectsFakeTimestamp[rxIndex].nanoseconds = timeStamp.nanoseconds;
...
private:
  ...
  // ---------------------------------------------------------------
  void HandleError(Can_Virtual_Controller* ctrl) {
    Can_Hwp_Frame frame;
    switch(ctrl->m_Error){
      case CAN_ERROR_BIT_MONITORING1:
        ctrl->UpCall(CAN_HWP_ERROR_BIT_MONITORING1, 0U, &frame);
        break;
      case CAN_ERROR_BIT_MONITORING0:
        ctrl->UpCall(CAN_HWP_ERROR_BIT_MONITORING0, 0U, &frame);
        break;
      case CAN_ERROR_BIT:
        ctrl->UpCall(CAN_HWP_ERROR_BIT, 0U, &frame);
        break;
      case CAN_ERROR_CHECK_ACK_FAILED:
        ctrl->UpCall(CAN_HWP_ERROR_CHECK_ACK_FAILED, 0U, &frame);
        break;
      case CAN_ERROR_ACK_DELIMITER:
        ctrl->UpCall(CAN_HWP_ERROR_ACK_DELIMITER, 0U, &frame);
        break;
      case CAN_ERROR_ARBITRATION_LOST:
        ctrl->UpCall(CAN_HWP_ERROR_ARBITRATION_LOST, 0U, &frame);
        break;
      case CAN_ERROR_OVERLOAD:
        ctrl->UpCall(CAN_HWP_ERROR_OVERLOAD, 0U, &frame);
        break;
      case CAN_ERROR_CHECK_FORM_FAILED:
        ctrl->UpCall(CAN_HWP_ERROR_CHECK_FORM_FAILED, 0U, &frame);
        break;
      case CAN_ERROR_CHECK_STUFFING_FAILED:
        ctrl->UpCall(CAN_HWP_ERROR_CHECK_STUFFING_FAILED, 0U, &frame);
        break;
      case CAN_ERROR_CHECK_CRC_FAILED:
        ctrl->UpCall(CAN_HWP_ERROR_CHECK_CRC_FAILED, 0U, &frame);
        break;
      case CAN_ERROR_BUS_LOCK:
        ctrl->UpCall(CAN_HWP_ERROR_BUS_LOCK, 0U, &frame);
        break;
      default:
        ctrl->UpCall(CAN_HWP_ERROR_NO_ERROR, 0U, &frame);
        break;
    }
    ctrl->m_Error = 0U; // reset error
  }
  ...
  void ReceiveFrame(Can_Virtual_Controller* ctrl, Can_Virtual_Controller* sourceCtrl, Can_Hwp_Frame* frame) {
    ...
    if(CAN_ID_FD_BIT_SET(frame->id)) { /* [$CAN 1162] [$CAN21 486] [$CAN 1229] [$CAN 1230] */
      if ((ctrl->m_FdBaudrate != sourceCtrl->m_FdBaudrate) && (sourceCtrl->m_FdTxBitRateSwitch == TRUE)) {
    ...
    if (ctrl->m_CurrentState == CAN_HWP_EVENT_SLEEPING) {
      ...
      for (unsigned j = 0U; j < ctrl->m_NumRxObjects; j++) {
        ...
        if ((rxObj.idType == CAN_HWP_IDTYPE_STANDARD && !isExtended) ||
            (rxObj.idType == CAN_HWP_IDTYPE_EXTENDED && isExtended) ||
            (rxObj.idType == CAN_HWP_IDTYPE_MIXED)) {

          // [$CAN 1301]
          if(Can_FramePassesFiltering(frameId, &rxObj)) {
            if (frame->length > rxObj.objectPayloadLength) { /* [$CAN21 9512] */
              mcal_printf("VirtualCan", "Receive frame dropped: Hrh: %u frame length: %u objectPayloadLength: %u\n",  rxObj.handle, frame->length, rxObj.objectPayloadLength);
              return;
            }
            ...
            // [$CANHWP 33] [$CANHWP 36]
            ctrl->UpCall(CAN_HWP_EVENT_RX, rxObj.handle, &rxFrame);

            // Logically, a CAN controller either accepts a frame or it doesn't - it shouldn't accept a frame multiple
            // times. As soon as we find one hardware object with filters configured to accept the incoming frame, we
  ...
  void WaitForEvent(int* eventPipe) {
      ...
      if (fdReady > 0 && FD_ISSET(eventPipe[0], &readFds)) {
        char buffer[512];
        ssize_t ret;  // introduced ret as didn't want to disable [-Wunused-result] and [-Wunused-but-set-variable] for other cases
        ret = read(eventPipe[0], buffer, sizeof(buffer));
        ret++;
      }
  }
  ...
  void SendEvent(int* eventPipe) {
    char dummy = 0;
    ssize_t ret;  // introduced ret as didn't want to disable [-Wunused-result] and [-Wunused-but-set-variable]
    vrtaEnterUninterruptibleSection();
    ret = write(eventPipe[1], &dummy, sizeof(char));
    vrtaLeaveUninterruptibleSection();
    ret++;
  }
...
public:
  ...
  Std_ReturnType GetControllerRxErrorCounter(uint8* RxErrorCounterPtr) {
    return Can_Virtual.GetControllerRxErrorCounter(m_HwId, RxErrorCounterPtr);
  }

  void InjectControllerRxErrorCount(uint8 RxErrorCount) {
    Can_Virtual.InjectControllerRxErrorCount(m_HwId, RxErrorCount);
  }

  Std_ReturnType GetControllerTxErrorCounter(uint8* TxErrorCounterPtr) {
    return Can_Virtual.GetControllerTxErrorCounter(m_HwId, TxErrorCounterPtr);
  }

  void InjectControllerTxErrorCount(uint8 TxErrorCount) {
    Can_Virtual.InjectControllerTxErrorCount(m_HwId, TxErrorCount);
  }

  void ForceError(uint8 error) {
    Can_Virtual.ForceError(m_HwId, error);
  }

  Std_ReturnType ReadCurrentTime(Can_TimeStampType* timeStampPtr, boolean fromAction) {
    return Can_Virtual.CurrentTime(m_HwId, timeStampPtr, fromAction);
  }

  Std_ReturnType InjectCurrentTime(Can_TimeStampType timeStamp) {
    return Can_Virtual.InjectCurrentTime(m_HwId, timeStamp);
  }

  Std_ReturnType EnableEgressTimeStamp(Can_HwHandleType Hth)  {
    return Can_Virtual.EnableEgressTimeStamp(m_HwId, Hth);
  }

  Std_ReturnType InjectEnableEgressTimeStamp(Can_HwHandleType Hth, boolean enabled)  {
    return Can_Virtual.InjectEnableEgressTimeStamp(m_HwId, Hth, enabled);
  }

  Std_ReturnType GetEgressTimeStamp(PduIdType TxPduId, Can_HwHandleType Hth, Can_TimeStampType* timeStampPtr)  {
    return Can_Virtual.GetEgressTimeStamp(m_HwId, TxPduId, Hth, timeStampPtr);
  }

  Std_ReturnType InjectEgressTimeStamp(PduIdType TxPduId, Can_HwHandleType Hth, Can_TimeStampType timeStamp)  {
    return Can_Virtual.InjectEgressTimeStamp(m_HwId, TxPduId, Hth, timeStamp);
  }

  Std_ReturnType GetIngressTimeStamp(Can_HwHandleType Hrh, Can_TimeStampType* timeStampPtr) {
    return Can_Virtual.GetIngressTimeStamp(m_HwId, Hrh, timeStampPtr);
  }

  Std_ReturnType InjectIngressTimeStamp(Can_HwHandleType Hrh, Can_TimeStampType timeStamp) {
    return Can_Virtual.InjectIngressTimeStamp(m_HwId, Hrh, timeStamp);
  }
};

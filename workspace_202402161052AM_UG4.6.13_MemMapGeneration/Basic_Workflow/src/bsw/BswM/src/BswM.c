...
#if (defined(BSWM_NO_OF_ERP_REQ) && (BSWM_NO_OF_ERP_REQ > 0 ))
            #if (defined(BSWM_NO_OF_NM_CHANNELS) && (BSWM_NO_OF_NM_CHANNELS > 0 ))
                  uint16 BswM_NmERP_idx_u16 = 0;
            #endif /* (BSWM_NO_OF_NM_CHANNELS) */
#endif /* ( BSWM_NO_OF_ERP_REQ ) */
    ...
    else
    {
        ...
        if(FALSE != isValidConfigPtr_b)
        {
                    ...
                    && (BSWM_SW_MINOR_VERSION == ConfigPtr->dataVersionInfo_st.sw_minor_version))
            {
                ...
                /* Initialize all the BswMEventRequestPorts to BSWM_EVENT_IS_CLEARED state */
                #if (defined(BSWM_NO_OF_ERP_REQ) && (BSWM_NO_OF_ERP_REQ > 0))
                        #if(defined(BSWM_NO_OF_ERP_COMM_INITIATE_RESET) && (BSWM_NO_OF_ERP_COMM_INITIATE_RESET > 0))
                                 BswM_Cfg_ComMInitiateResetModeInfo_ast[0].dataMode_en = BSWM_EVENT_IS_CLEARED;
                        #endif
                        #if(defined(BSWM_NO_OF_ERP_DCM_APP_UPDATE_INDICATION) && (BSWM_NO_OF_ERP_DCM_APP_UPDATE_INDICATION > 0))
                                 BswM_Cfg_DcmApplicationUpdatedindicationModeInfo_ast[0].dataMode_en = BSWM_EVENT_IS_CLEARED;
                        #endif
                        #if(defined(BSWM_NO_OF_NM_CHANNELS) && (BSWM_NO_OF_NM_CHANNELS > 0))
                            for (BswM_NmERP_idx_u16 = 0; BswM_NmERP_idx_u16 < BSWM_NO_OF_NM_CHANNELS; BswM_NmERP_idx_u16++)
                               {
                                    BswM_Cfg_NmWKupIndInfo_ast[BswM_NmERP_idx_u16].dataMode_en = BSWM_EVENT_IS_CLEARED;
                               }
                        #endif
                #endif

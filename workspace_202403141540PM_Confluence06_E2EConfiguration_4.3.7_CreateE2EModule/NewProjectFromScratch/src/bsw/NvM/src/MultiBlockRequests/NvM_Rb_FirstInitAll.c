                                                         ...
                                                         NvM_Prv_Service_InitiationErrors_tuo* Errors_puo)
{
    ...
    return (NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo, NVM_PRV_BLOCK_FLAG_SELECT_FOR_FIRST_INIT_ALL));
}
...
static void NvM_Prv_FirstInitAll_FindBlocks(void)
{
    ...
    // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2830] Iterate over all used blocks to find the blocks
    ...
    for (idBlock_uo = NVM_CFG_FIRST_USED_BLOCK; idBlock_uo < NVM_CFG_NR_BLOCKS; ++idBlock_uo)
    {
        ...
        if (NvM_Prv_BlkDesc_GetSize(idBlock_uo) > 0u)
        {
            ...
            // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2830] Only initialize blocks which are selected for FirstInitAll
            // Further conditions are already checked in the configuration scripts
            if (NvM_Prv_FirstInitAll_IsBlockParticipating(idBlock_uo, &Errors_uo))
            {

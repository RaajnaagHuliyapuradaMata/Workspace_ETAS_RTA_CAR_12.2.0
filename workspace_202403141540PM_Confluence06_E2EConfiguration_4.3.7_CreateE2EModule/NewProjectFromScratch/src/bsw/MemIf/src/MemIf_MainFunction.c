...
void MemIf_Cancel(uint8 DeviceIndex)
{
    ...
    {
        Fee_Rb_Idx_Cancel(Fee_Rb_GetDeviceNameFromDeviceIndex(DeviceIndex), FEE_RB_USER_NVM_E);
    }
#else
    if (DeviceIndex == 0u)
    {
...
void MemIf_CheckDeviceIndex(uint8 DeviceIndex)
{
    ...
    {
        MemIf_Prv_DetReportError(MEMIF_SID_CHECK_INDEX, MEMIF_E_PARAM_DEVICE);
    }

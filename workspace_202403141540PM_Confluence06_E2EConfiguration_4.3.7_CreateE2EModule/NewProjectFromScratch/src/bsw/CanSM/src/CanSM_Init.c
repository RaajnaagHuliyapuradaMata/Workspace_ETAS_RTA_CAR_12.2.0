...
boolean CanSM_NoComRequested_ab[CANSM_NUM_CAN_NETWORKS];
...
void CanSM_Init(const CanSM_ConfigType* ConfigPtr)
{
    ...
    for(CanSM_NetworkIdx_u8 =0; CanSM_NetworkIdx_u8 < CanSM_ConfigSet_pcst->CanSM_SizeOfCanSMNetworks_u8 ; CanSM_NetworkIdx_u8++)
    {
        ...
        /* Initialize No com request status */
        CanSM_NoComRequested_ab[CanSM_NetworkIdx_u8] = FALSE;

        /*When transceiver is configured, also check if PN is supported*/
        if(CANSM_INVALID_TRCV_HNDLE != CanSM_Network_pcst[CanSM_NetworkIdx_u8].Trcv_hndle_u8)
            {
...
/* HIS METRIC v(G) VIOLATION IN CanSM_DeInitPnNotSupported: Function contains very simple "else if"
   statements and "switch-cases", HIS metric compliance would decrease readability and maintainability. */
...
***************************************************************************************************/
#if(CANSM_PN_SUPPORT_CONFIGD == STD_ON)
/* HIS METRIC PATH,v(G),STMT VIOLATION IN CanSM_DeInitPnSupported: Function contains very simple

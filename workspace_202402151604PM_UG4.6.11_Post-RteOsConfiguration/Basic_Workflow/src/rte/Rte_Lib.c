/** @file         Rte_Lib.c
  ...
  *               RTE_INITIALIZING                       = 0
  *               RTE_REQUIRES_IOC                       = 0
  *               RTE_CLUSTER_APPLICATION_MODE_ENABLED   = 0
  *               RTE_DEVIATE_LDCOM_ALIVE_TIMEOUT        = 0
  *               RTE_RIPS_SUPPORT                       = 0
  *               RTE_USE_GETCURRENTTASKALARMINDEX       = 0
  *               RTE_CALPRM_INITRAM_DEFAULTS            = 0
  *               RTE_CHECK_LDCOM_ALIVE_TIMEOUTS_DEFINED = 0
  *
...
/* [$Satisfies $rte_resource 047] */
VAR(uint16, RTE_DATA) Rte_ResourceCount  = 0U;
                ...
                Rte_ConstDataPtr   data )
{
   ...
   /* protect queue access applying an appropriate mechanism */
   Rte_WriteQueue_GetProtection(q);
   ...
   /* release the applied protection mechanism */
   Rte_WriteQueue_ReleaseProtection(q);

   return status;
}

/* --------------------------------------------------------------------------- */
/** @brief  Select and appropriate resource protection mechanism based on the 
  *         event type, called from Rte_WriteQueue.
  *
  * @param[in] q        Handle of queue common data structure
  *
  * @retval void
  */
FUNC(void, RTE_CODE)
Rte_WriteQueue_GetProtection(Rte_QCmnRefType q)
{
   switch ( q->notification_type )
   {
      default:
...
/* --------------------------------------------------------------------------- */
/** @brief  Release the selected resource protection mechanism based on the 
  *         event type, called from Rte_WriteQueue.
  *
  * @param[in] q        Handle of queue common data structure
  *
  ...
  */
FUNC(void, RTE_CODE)
Rte_WriteQueue_ReleaseProtection(Rte_QCmnRefType q)
{
   switch ( q->notification_type )
   {
      default:

/*
 **********************************************************************************************************************
 *
 * Copyright (c) ETAS GmbH 2023, all rights reserved.
 * The reproduction, distribution and utilization of this document as well as the communication of its contents to
 * others without explicit authorization is prohibited. Offenders will be held liable for the payment of damages.
 * All rights reserved in the event of the grant of a patent, utility model or design.
 *
 **********************************************************************************************************************
 
 **********************************************************************************************************************
 Machine Type: MCU_RB_GENERIC
 Generated by MEMMAPGEN 12.2.0  
 **********************************************************************************************************************
*/ 
/* MR12 DIR 4.10 VIOLATION: Memmap file must be included multiple times */
/* MR12 RULE 20.5 VIOLATION: AUTOSAR MemMap concept requires #undef, AUTOSAR MemMap requirements are incompatible to MISRA */ 
#if defined COM_START_SEC_CODE
  /* CODE pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_CODE
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_CODE           
#elif defined COM_STOP_SEC_CODE      
  /* CODE pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_CODE
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_CODE             
#elif defined COM_START_SEC_CONFIG_DATA_16
  /* CONFIG_DATA_16 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_CONFIG_DATA_16
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_CONFIG_DATA_16           
#elif defined COM_STOP_SEC_CONFIG_DATA_16      
  /* CONFIG_DATA_16 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_CONFIG_DATA_16
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_CONFIG_DATA_16             
#elif defined COM_START_SEC_CONFIG_DATA_32
  /* CONFIG_DATA_32 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_CONFIG_DATA_32
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_CONFIG_DATA_32           
#elif defined COM_STOP_SEC_CONFIG_DATA_32      
  /* CONFIG_DATA_32 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_CONFIG_DATA_32
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_CONFIG_DATA_32             
#elif defined COM_START_SEC_CONFIG_DATA_64
  /* CONFIG_DATA_64 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_CONFIG_DATA_64
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_CONFIG_DATA_64           
#elif defined COM_STOP_SEC_CONFIG_DATA_64      
  /* CONFIG_DATA_64 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_CONFIG_DATA_64
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_CONFIG_DATA_64             
#elif defined COM_START_SEC_CONFIG_DATA_8
  /* CONFIG_DATA_8 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_CONFIG_DATA_8
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_CONFIG_DATA_8           
#elif defined COM_STOP_SEC_CONFIG_DATA_8      
  /* CONFIG_DATA_8 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_CONFIG_DATA_8
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_CONFIG_DATA_8             
#elif defined COM_START_SEC_CONFIG_DATA_UNSPECIFIED
  /* CONFIG_DATA_UNSPECIFIED pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_CONFIG_DATA_UNSPECIFIED
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_CONFIG_DATA_UNSPECIFIED           
#elif defined COM_STOP_SEC_CONFIG_DATA_UNSPECIFIED      
  /* CONFIG_DATA_UNSPECIFIED pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_CONFIG_DATA_UNSPECIFIED
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_CONFIG_DATA_UNSPECIFIED             
#elif defined COM_START_SEC_CONST_UNSPECIFIED
  /* CONST_UNSPECIFIED pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_CONST_UNSPECIFIED
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_CONST_UNSPECIFIED           
#elif defined COM_STOP_SEC_CONST_UNSPECIFIED      
  /* CONST_UNSPECIFIED pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_CONST_UNSPECIFIED
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_CONST_UNSPECIFIED             
#elif defined COM_START_SEC_VAR_CLEARED_16
  /* VAR_CLEARED_16 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_VAR_CLEARED_16
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_VAR_CLEARED_16           
#elif defined COM_STOP_SEC_VAR_CLEARED_16      
  /* VAR_CLEARED_16 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_VAR_CLEARED_16
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_VAR_CLEARED_16             
#elif defined COM_START_SEC_VAR_CLEARED_32
  /* VAR_CLEARED_32 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_VAR_CLEARED_32
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_VAR_CLEARED_32           
#elif defined COM_STOP_SEC_VAR_CLEARED_32      
  /* VAR_CLEARED_32 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_VAR_CLEARED_32
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_VAR_CLEARED_32             
#elif defined COM_START_SEC_VAR_CLEARED_64
  /* VAR_CLEARED_64 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_VAR_CLEARED_64
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_VAR_CLEARED_64           
#elif defined COM_STOP_SEC_VAR_CLEARED_64      
  /* VAR_CLEARED_64 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_VAR_CLEARED_64
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_VAR_CLEARED_64             
#elif defined COM_START_SEC_VAR_CLEARED_8
  /* VAR_CLEARED_8 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_VAR_CLEARED_8
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_VAR_CLEARED_8           
#elif defined COM_STOP_SEC_VAR_CLEARED_8      
  /* VAR_CLEARED_8 pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_VAR_CLEARED_8
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_VAR_CLEARED_8             
#elif defined COM_START_SEC_VAR_CLEARED_UNSPECIFIED
  /* VAR_CLEARED_UNSPECIFIED pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_VAR_CLEARED_UNSPECIFIED
  #include "Bsw_MemMap.h"
			 
  #undef COM_START_SEC_VAR_CLEARED_UNSPECIFIED           
#elif defined COM_STOP_SEC_VAR_CLEARED_UNSPECIFIED      
  /* VAR_CLEARED_UNSPECIFIED pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_VAR_CLEARED_UNSPECIFIED
  #include "Bsw_MemMap.h"
			 
  #undef COM_STOP_SEC_VAR_CLEARED_UNSPECIFIED             
#elif defined Com_START_SEC_CODE
  /* CODE pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_START_SEC_CODE
  #include "Bsw_MemMap.h"
			 
  #undef Com_START_SEC_CODE           
#elif defined Com_STOP_SEC_CODE      
  /* CODE pragma configuration not available, please update the respective template file */ 
			 
  #define BSW_STOP_SEC_CODE
  #include "Bsw_MemMap.h"
			 
  #undef Com_STOP_SEC_CODE             
#else 
#error "Com_MemMap.h, wrong pragma command" 
#endif

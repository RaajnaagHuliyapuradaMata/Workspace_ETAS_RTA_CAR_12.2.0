...
/* to get the position of the TxPduId in the mapping table*/
    ...
    else                                   /*Buffer is ready to store the incoming PDU*/
    {
        ...
        else    /*[SWS_CANIF_00836]*/
        {
            ...
            CanIf_Prv_TxBufferRam_ast[lBufferId].last_index = (((CanIf_Prv_TxBufferRam_ast[lBufferId].last_index) + 1u)&(0xFFu));
            ...
            if(lCanIdWriteIndex == ((lBufferSize - 1u)&0xFFu))
            {
                CanIf_Prv_TxBufferRam_ast[lBufferId].bufferstatus = CANIF_TXBUFFER_FULL;
            }
            ...
            *(lDataBuf_p + ((lSduWriteIndex)*(lDataMaxLen+1u)) + 0u)          = 0x00u;
...
/*For trigger transmit pdu, no need to store data*/
...
#endif
        {
            ...
            for(lDataIndex=1; lDataIndex < (lDataLen + 1u); lDataIndex++)   /*lDataLen is excluded of flag index*/
            {
                /*Copy the data to SDU buffer*/
                *(lDataBuf_p + ((lSduWriteIndex)*(lDataMaxLen+1u)) + lDataIndex) = *(lSduPtr+(lDataIndex-1u));
            }
        ...
        CanIf_Cfg_CanIdBuffer_tst * CanIdBuf_p, uint8 BufferSize)
{
    ...
    for (lIndex1 = 0u; lIndex1 <= CanIf_Prv_TxBufferRam_ast[BufferId].last_index; lIndex1++)
    {
        for (lIndex2 = 0u; (lIndex2 <= ((CanIf_Prv_TxBufferRam_ast[BufferId].last_index)-lIndex1)) && ((lIndex2+1u)<BufferSize); lIndex2++)
        {
...
void CanIf_Prv_ClearTxChannelBuffer(uint8 ControllerId)
{
    ...
    for(BufferIdArrayIndex = 0; BufferIdArrayIndex < (lCtrlConfig_pst->TotalBufferCount); BufferIdArrayIndex++)
    {
        ...
        if (CanIf_Prv_TxBufferRam_ast[lBufferId].bufferstatus != CANIF_TXBUFFER_EMPTY)       /*Buffer is not empty*/
        {
            for(BufferIndex = 0 ; BufferIndex < lBufferSize ; BufferIndex++ )
            {
                ...
                *(lDataBuf_p + ((BufferIndex)*(lDataMaxLen+1u)) + 0u)    = 0xFFu; /*Free*/
                for(lDataIndex=1u; lDataIndex < (lDataMaxLen + 1u); lDataIndex++)
                {
                    *(lDataBuf_p + ((BufferIndex)*(lDataMaxLen + 1u)) + lDataIndex) = 0u;
                }

                (lCanIdBuf_p + BufferIndex)->BufferIndex = 0u;
                (lCanIdBuf_p + BufferIndex)->CanId = 0u;
                (lCanIdBuf_p + BufferIndex)->SduLength = 0u;
                (lCanIdBuf_p + BufferIndex)->swPduHandle = 0u;
            }
            CanIf_Prv_TxBufferRam_ast[lBufferId].last_index   = 0xFFu;                  /*Important. Don't change line of code*/
    ...
    for(TxPduIdArrayIndex = 0u ; TxPduIdArrayIndex < (lCtrlConfig_pst->TotalTxPduCount) ; TxPduIdArrayIndex++ )
    {
	...
	for ( lBufferArrayIndex=0; lBufferArrayIndex < CANIF_TOTAL_TXBUFFERS; lBufferArrayIndex++ )
	{
	        ...
	        ( lBufferConfig_pst->CanIf_HthConfigPtr->CanHandleType == CANIF_BASIC  ) )
	    {
            ...
            if ( CanIf_Prv_TxBufferRam_ast[lBufferId].bufferstatus != CANIF_TXBUFFER_EMPTY )
            {
                ...
                {
                    /* Clear the Data in DataBuf*/
                    *(lDataBuf_p + ((BufferIndex)*(lDataMaxLen+1)) + 0u) = 0xFFu; /*Free*/
                    for( lDataIndex=1u; lDataIndex < (lDataMaxLen + 1u); lDataIndex++ )
                    {
                       *(lDataBuf_p + ((BufferIndex)*(lDataMaxLen + 1u)) + lDataIndex) = 0u;
                    }
                ...
                CanIf_Prv_TxBufferRam_ast[lBufferId].last_index    = 0xFFu;
                CanIf_Prv_TxBufferRam_ast[lBufferId].bufferstatus  = CANIF_TXBUFFER_EMPTY;
            } /* end of if ( CanIf_Prv_TxBufferRam_ast[lBufferId].bufferstatus != CANIF_TXBUFFER_EMPTY ) */

            for( lHthTxPduMapTblArryIndex = 0u;
...
void CanIf_Prv_BufferInit(void)
{
    ...
    /*For all buffers*/
     for(lBufferCustId = 0u; lBufferCustId < CanIf_Prv_ConfigSet_tpst->NumOfTxBuffers; lBufferCustId++)
     {
     ...
     for(TxPduId = 0u ; TxPduId < (CanIf_Prv_ConfigSet_tpst->NumOfTxPdus) ; TxPduId++ )
     {
    ...
    else                                         /*Buffer is NOT empty*/
    {
        /*Read CanId and TxPduId from CanId queue buffer*/
...
#endif
          {
              ...
              for(lDataIndex=1u; lDataIndex < (lDataLen + 1u); lDataIndex++)
              {
                  *(lSduPtr + (lDataIndex-1u)) = *(lDataBuf_p + ((lSduReadIndex)*(lDataMaxLen+1u)) + lDataIndex);
              }
...

{
    ...
    for(Index = 0u; Index < lBufferSize; Index++)
    {
        if(*(lDataBuf_p + ((Index)*(lDataMaxLen+1u)) + 0u) == 0xFFu)
        {
            *lFreeIndex = Index;
            break;
        }
                                               ...
                                               )
{
    ...
    if(RxPduCfg_pcst->RxBuffer_u16 != (uint16)0xFFFFu)
    {
        ...
        else
        {
            ...
            #endif /* #if(CANIF_METADATA_SUPPORT == STD_ON) */
            /* Copy the SduLength */
            *(RxTemBuff_pu8) = (uint8)(ConfigRxPduDlc_u8);
        }
                                            ...
                                            const CanIf_Cfg_RxPduType_tst * RxPduCfgPtr)
{
    ...
    /*varaiable to hold length*/
     uint8 SduLength_u8 = 0u;
...
#if(CANIF_METADATA_SUPPORT == STD_ON)
    metaData_p = PduInfoPtr->MetaDataPtr;
#endif
    ...
    if(RxPduCfgPtr->RxBuffer_u16 != 0xFFFFu)
    {
        ...
        /* Copy the data */
        for(Index_u16 = 1u; Index_u16 <= PduInfoPtr->SduLength; Index_u16++ )
        {
...
#if(CANIF_METADATA_SUPPORT == STD_ON)
     if(RxPduCfgPtr->MetadataLength_u8 >= 1u)
      {
         SduLength_u8 = (uint8)(PduInfoPtr->SduLength + RxPduCfgPtr->MetadataLength_u8);
        /*Get the MetaData*/
        for(Index_u16 = PduInfoPtr->SduLength+1u; Index_u16 <= SduLength_u8;Index_u16++)
        {
            *metaData_p = *(RxBuffPtr_Temp + Index_u16);
             metaData_p++;
        }
      }
#else

...
/* Below commented code needs to be uncommented next release*/
...
#if (CANIF_RBA_J1939FDTP_ENABLED != STD_OFF)
#include "rba_J1939FdTp.h"
#include "rba_J1939FdTp_Cbk.h"
/*#if (!defined(RBA_J1939FDTP_AR_RELEASE_MAJOR_VERSION) || (RBA_J1939FDTP_AR_RELEASE_MAJOR_VERSION != CANIF_AR_RELEASE_MAJOR_VERSION))
#error "AUTOSAR major version undefined or mismatched"
#endif
#if (!defined(RBA_J1939FDTP_AR_RELEASE_MINOR_VERSION) || (RBA_J1939FDTP_AR_RELEASE_MINOR_VERSION != CANIF_AR_RELEASE_MINOR_VERSION))
#error "AUTOSAR minor version undefined or mismatched"
#endif*/
#endif
...
/* Controller structure to store Run time value */
typedef struct
{
    /*This element stores Controller and PDU mode for a controller. Most Signifcant Nibble gives Controller mode and
     * Least Significant Nibble gives PDU mode */
    /* CtrlMode: CAN_CS_UNINIT(0), CAN_CS_STARTED(1), CAN_CS_STOPPED(2), CAN_CS_SLEEP(3)
     * PduMode: CANIF_OFFLINE(0), CANIF_TX_OFFLINE(1), CANIF_TX_OFFLINE_ACTIVE(2), CANIF_ONLINE(3),
     *          CANIF_TX_TP_ONLINE(4), CANIF_TX_USER_TP_ONLINE(5)
     * E.g: Ctrl_Pdu_mode: 0x34- CAN_CS_SLEEP and CANIF_TX_TP_ONLINE */
...
#if(CANIF_PUBLIC_TXBUFFERING == STD_ON)
extern CanIf_Prv_TxBufferStatus_tst CanIf_Prv_TxBufferRam_ast[CANIF_TOTAL_TXBUFFERS];

extern CanIf_Prv_TxPduStatus_tst CanIf_Prv_TxPduRam_ast[CANIF_TOTAL_TXPDUS];
#endif
 ...
 */
extern CanIf_ControllerStateType CanIf_Prv_ControllerState_ast[CANIF_TOTAL_CTRLS];
...
extern CanIf_NotifStatusType CanIf_Prv_TxNotification_aen[CANIF_TOTAL_TXPDUS];
#endif
...
extern uint32 CanIf_DynTxPduCanId_au32[CANIF_TOTAL_DYNAMIC_PDUS];
#endif

#if (CANIF_RB_CHANGERXPDUID_API != STD_OFF)
extern uint32 CanIf_DynRxPduCanId_au32[CANIF_RX_DYNAMIC_PDUS];
#endif
...
/* Service Id of CanIf_GetCurrentTime API */
#define CANIF_GET_CURRENT_TIME_SID                      0x51
/* Service Id of CanIf_EnableEgressTimeStamp API */
#define CANIF_ENABLE_EGRESS_TIME_STAMP_SID              0x52
/* Service Id of CanIf_GetEgressTimeStamp API */
#define CANIF_GET_EGRESS_TIME_STAMP_SID                 0x53
/* Service Id of CanIf_GetIngressTimeStamp API */
#define CANIF_GET_INGRESS_TIME_STAMP_SID                0x54
#endif
...
/*CAN Interface controller mode state machine is not in mode CAN_CS_SLEEP*/
...
/*CAN Interface controller mode state machine is in mode CAN_CS_STOPPED*/
...
#define CANIF_META_DATA_BUFF_SIZE       32u
...
#define CANIF_DET_REPORT_ERROR_NO_CONDITON(API, ERROR)                                                       \
  ...
  /*init requirement*/
/* Macro For Det with return type as  'Std_ReturnType' */
   #define CANIF_DET_REPORT_ERROR_INIT_NOT_OK(CONDITION, API, ERROR)                                                   \
                                  if(CONDITION)                                                                \
                                  {                                                                            \
                                      (void)Det_ReportError((CANIF_MODULE_ID), (CANIF_INSTANCE_ID), (API), (ERROR));\
                                       return E_NOT_OK;                                                        \
...
#if ((CANIF_PUBLIC_TXCONFIRM_POLLING_SUPPORT != STD_OFF) || (CANIF_PUBLIC_READRXPDU_NOTIFY_STATUS_API != STD_OFF)          \
                                                                       ||(CANIF_READTXPDU_NOTIFY_STATUS_API != STD_OFF))
        #define CANIF_DET_REPORT_ERROR_INIT_NO_NOTIF(CONDITION, API, ERROR)                                          \
                                   if(CONDITION)                                                                \
                                   {                                                                            \
                                       (void)Det_ReportError((CANIF_MODULE_ID), (CANIF_INSTANCE_ID), (API), (ERROR));\
                                        return CANIF_NO_NOTIFICATION;                                           \
...
/*init*/
      #define CANIF_DET_REPORT_ERROR_INIT_VOID_RET(CONDITION, API, ERROR)                                                  \
                                  if(CONDITION)                                                                \
                                  {                                                                            \
                                      (void)Det_ReportError((CANIF_MODULE_ID), (CANIF_INSTANCE_ID), (API), (ERROR));\
                                      return;                                                                       \
...
#define CANIF_DET_RUN_TIME_REPORT_ERROR(CONDITION, API, ERROR)                                     \
        if(CONDITION)                                                                \
        {                                                                            \
            (void)Det_ReportRuntimeError((CANIF_MODULE_ID), (CANIF_INSTANCE_ID), (API), (ERROR));                                                       \
        }

/* Macro For Run time Det with return type as  'Std_ReturnType' */
#define CANIF_DET_RUN_TIME_REPORT_ERROR_NOT_OK(CONDITION, API, ERROR)                                                   \
                                  if(CONDITION)                                                                \
                                  {                                                                            \
                                      (void)Det_ReportRuntimeError((CANIF_MODULE_ID), (CANIF_INSTANCE_ID), (API), (ERROR));\
                                       return E_NOT_OK;                                                        \
...
/*Macro for Run time DET which returns from the function on invocation with void return type*/
#define CANIF_DET_RUN_TIME_REPORT_ERROR_VOID_RET(CONDITION, API, ERROR)                                                  \
                                   if(CONDITION)                                                                \
                                   {                                                                            \
                                       (void)Det_ReportRuntimeError((CANIF_MODULE_ID), (CANIF_INSTANCE_ID), (API), (ERROR));\
                                       return;                                                                       \

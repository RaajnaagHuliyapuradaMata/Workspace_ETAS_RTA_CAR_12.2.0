...
void CANIF_RXINDICATION_FNAME( const Can_HwType * Mailbox,
...
/*local variable to store CAN Identifier of a receive CAN L-PDU */
...
#if( CANIF_CFG_PUBLIC_MULTIPLE_DRIVER_SUPPORT == STD_ON || CANIF_DEV_ERROR_DETECT == STD_ON )
    ...
    /*Data buffer to store payload data */
    uint8 SduDataBuffer_au8[SDU_DATA_BUFF_SIZE] = {0};
   /*Data buffer to store metadata CanIf bytes*/
    uint8 metaDataBuffer_au8[CANIF_META_DATA_BUFF_SIZE] = {0};
    ...
    CANIF_DET_REPORT_ERROR_INIT_VOID_RET(CanIf_Prv_InitStatus_b == FALSE, CANIF_RX_INDICATION_SID, CANIF_E_UNINIT);
    ...
    if((uint8)CAN_CS_STARTED == (((ControllerState_ps->Ctrl_Pdu_mode)&CANIF_BIT_MASK_PDU_MODE)>>CANIF_Ctrl_BIT_SHIFT))
    {
        /*Check if the channel mode is enabled in receive path*/
        if(((uint8)CANIF_TX_OFFLINE == ((ControllerState_ps->Ctrl_Pdu_mode)&CANIF_BIT_MASK_CTRL_MODE))||((uint8)CANIF_ONLINE == ((ControllerState_ps->Ctrl_Pdu_mode)&CANIF_BIT_MASK_CTRL_MODE))||((uint8)CANIF_TX_OFFLINE_ACTIVE == ((ControllerState_ps->Ctrl_Pdu_mode)&CANIF_BIT_MASK_CTRL_MODE)))
        {
    ...
    }/* end of if(CAN_CS_STARTED == ControllerMode) */
        ...
        #endif
        {
            ...
            #endif
            {
               ...
               RxPduCfg_pcst = CanIf_Prv_ConfigSet_tpst->RxPduConfig_pcst + PduId_qu8;
                ...
                CANIF_DET_RUN_TIME_REPORT_ERROR_VOID_RET((PduInfoPtr->SduLength < lConfigRxPduDlc), CANIF_RX_INDICATION_SID, CANIF_E_INVALID_DATA_LENGTH)
                ...
                #endif /* End of #if(CANIF_CFG_DLC_CHECK == STD_ON) */
                {
                    ...
                    if(RxPduCfg_pcst->MetadataLength_u8 >= 1u)
                    {
                        ...
                        for(Index_u8 = 0; Index_u8 < (uint8)RxPduCfg_pcst->MetadataLength_u8; Index_u8++)
                        {
                            ...
                            metaDataBuffer_au8[Index_u8]= (uint8)(CanIdNoIdType_t & CANIF_GET_ONE_BYTE);
                            CanIdNoIdType_t = (CanIdNoIdType_t>>SHIFT_8);
                        }
                    }
                    PduInfoTemp_tst.MetaDataPtr = (uint8*)(&metaDataBuffer_au8[0]);
                    PduInfoTemp_tst.SduDataPtr = (uint8*)(&SduDataBuffer_au8[0]);

                    #else
                    PduInfoTemp_tst.SduDataPtr =(uint8*)(PduInfoPtr->SduDataPtr);
                    PduInfoTemp_tst.MetaDataPtr =NULL_PTR;
                    #endif
                    #if CANIF_CFG_USER_RX_ENABLED == STD_ON
                    ULPduInfoTypUSER_tst.SduDataPtr= PduInfoTemp_tst.SduDataPtr;
                    ULPduInfoTypUSER_tst.MetaDataPtr= PduInfoTemp_tst.MetaDataPtr;
                    #endif
                    #if CANIF_CFG_UL_RX == STD_ON
                    ULPduInfoTyp_tst.SduDataPtr= PduInfoTemp_tst.SduDataPtr;
                    ULPduInfoTyp_tst.MetaDataPtr= PduInfoTemp_tst.MetaDataPtr;
                    #endif

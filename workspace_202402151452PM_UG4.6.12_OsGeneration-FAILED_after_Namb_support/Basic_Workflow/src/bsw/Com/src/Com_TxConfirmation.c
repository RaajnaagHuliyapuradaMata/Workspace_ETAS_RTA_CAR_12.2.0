...
void Com_Prv_InternalTxConfirmation(PduIdType idTxPdu_uo)
{
    ...
    * this send-request are received. */
    ...
    if (txIpduRamPtr_pst->cntrRepetitions_u8 == COM_ZERO)
    {
        /*This is checked in Com_MainFunctionTx()*/
        Com_SetRamValue(TXIPDU,_CONFIR,txIpduRamPtr_pst->txFlags_u16,COM_TRUE);
        ...
        * the I-PDU was successfully confirmed.*/
        ...
        if (Com_GetValue(TXIPDU,_CLRUPDBIT,constByteValue_u16) == COM_CLRUPDATEBIT_CONFIRMATION)
        {
...
# else
        Com_Prv_TxIPduAckCallbacks((Com_IpduId_tuo)idTxPdu_uo);

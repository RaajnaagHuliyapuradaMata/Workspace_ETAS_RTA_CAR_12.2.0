 ...
 **********************************************************************************************************************
 ...
 Description      : Called once upon reception of each segment. Within this call, the received data is copied to the
                    ...
                    returned by the previous API call indicates sufficient receive buffer
                    (SduLength = RxBufferSizePtr). The function must only be called if the connection has been
                    accepted by an initial call to Com_StartOfReception.
...
# endif /* end of COM_PRV_ERROR_HANDLING */
    {
        ...
        */
        ...
        if (Com_GetRamValue(RXIPDU,_LARGEDATAINPROG,rxIpduRamPtr_pst->rxFlags_u16))
        {
            PduLengthType rxIPduLengthCfg_uo;

            rxIPduLengthCfg_uo = COM_GET_RXIPDU_SIZE(idPdu_uo);
            ...
            if ((rxIpduRamPtr_pst->rxTPIPduLength_uo + pduInfoPtr_pcst->SduLength) <= rxIPduLengthCfg_uo)
            {
                ...
                *rxBufferSizePtr_puo = rxIPduLengthCfg_uo - rxIpduRamPtr_pst->rxTPIPduLength_uo;

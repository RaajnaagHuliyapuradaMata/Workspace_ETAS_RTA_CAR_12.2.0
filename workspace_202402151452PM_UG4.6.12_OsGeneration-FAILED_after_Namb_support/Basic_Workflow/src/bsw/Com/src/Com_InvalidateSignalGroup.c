...
uint8 Com_InvalidateSignalGroup(Com_SignalGroupIdType idSignalGroup_u16)
{
    ...
    /* return COM_SERVICE_NOT_AVAILABLE, if either
    * corresponding I-PDU group is stopped or
...
# endif /* end of COM_PRV_ERROR_HANDLING */
    {
        ...
        * in the COM internal buff-ers but the COM_SERVICE_NOT_AVAILABLE is returned */
        ...
        for ( ; idxIdGrpSig_qu16 < idMaxTxGrpSig_qu16; idxIdGrpSig_qu16++ )
        {
            ...
            * with the signal Id */
            if (Com_GetValue(TXGRPSIG,_INVACTION,txGrpSigConstPtr_pcst->txGrpSigFields_u8))
            {
                (void)Com_Prv_InternalInvalidateShadowSignal((Com_SignalIdType)idxIdGrpSig_qu16);
        ...
        if (invalidValueFound_u8 != COM_ZERO)
        {
            ...
            if (Com_GetRamValue(TXIPDU,_LARGEDATAINPROG,
                            COM_GET_TX_IPDU_CONSTDATA(txSigGrpConstPtr_pcst->idComIPdu_uo)->ipduRam_pst->txFlags_u16))
            {
                status_u8 = COM_BUSY;
    ...
    /* Note: if IPdu Group is stopped, then the Com_Prv_InternalSendSignalGroup( ) will

...
#define  COMM_GET_CUR_PNCCOMMODE_APIID              ((uint8)0x6a)
...
#define  COMM_BUSSM_BUSSLEEPMODE                    ((uint8)0x34)
...
#define COMM_PNC_SYN_SHUTDOWN_APIID                 ((uint8)0x6b)
...
/* Macro for DET Error report */
#if(COMM_DEV_ERROR_DETECT == STD_ON)

#define COMM_DET_REPORT_ERROR(ApiId, ErrorId)    \
        (void)Det_ReportError(COMM_MODULE_ID, COMM_INSTANCE_ID, (ApiId), (ErrorId))

#else

#define COMM_DET_REPORT_ERROR(ApiId, ErrorId) do {} while(0)
...
typedef struct
{
    ...
    boolean                     BusSMSleepIndicationStatus_b;           Indicates if Bus sleep is indicated by BusSM
    ...
    boolean                     ComMFullComWakeupRequestStatus_b;       Indicates if COMM_FULL_COMMUNICATION_WITH
                                                                        _WAKEUP_REQUEST is requested for a channel
...
typedef struct
{
    ...
    boolean                         BusSMSleepIndicationStatus_b;
...
#endif
    boolean                         ComMFullComWakeupRequestStatus_b;
} ComM_ChannelVarType_tst;
...
typedef struct
{
    ...
    uint8           numChannelsInFullCom_u8;    Holds the count of all channels(direct and through PNC) in Full
                                                com mode
    uint8           numChannelsInSilentCom_u8;  Holds the count of all channels(direct and through PNC) in silent
                                                com mode
...
typedef struct
{
    ...
    boolean             PncSynchronizeShutdownRequest_b; Synchronized Shutdown
}ComM_PncRamStructType_tst;
*/
...
typedef struct
{
    ...
    boolean             PncSynchronizeShutdownRequest_b;
    #endif
}ComM_PncRamStructType_tst;
...
typedef struct
{
    ...
    #if (COMM_SYNCHRONIZED_PNC_SHUTDOWN != STD_OFF)
    boolean  synchronizeShutdownRequest_b;
    #endif  /*  #if (COMM_SYNCHRONIZED_PNC_SHUTDOWN != STD_OFF)  */
} ComM_PncEventStructType_tst;

/* Note : Consider structure padding while modifying the below structure */
/*typedef struct {
    uint16                                       Inhibitioncounter_u16; Holds the Inhibition counter value
                                                 which has to be written to NvM
    uint8 ComMNoWakeup_u8[COMM_NVM_NO_WAKEUP_SIZE] ; Hols the ComMNowakeup value of all channels, which has to be
                                                 written to NvM
    uint8                                        ComMEcuGroupClassification_u8; Holds the Group classification value,
                                                 which has to be written to NvM
...
extern void ComM_Prv_TranslateInhibitionStatus(NetworkHandleType Channel_tu8,

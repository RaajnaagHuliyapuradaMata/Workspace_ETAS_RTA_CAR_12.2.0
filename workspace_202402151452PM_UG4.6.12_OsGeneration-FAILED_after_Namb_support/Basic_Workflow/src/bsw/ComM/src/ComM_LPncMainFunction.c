...
void ComM_Prv_PncMainFunction(PNCHandleType PncIndex_tu8)
{
    ...
    PncEventStruct_pst->synchronizeShutdownRequest_b = FALSE;
    #endif
    ...
    /* Inhibitions overrule user requests as well as pnc-user requests.
    * If any of the inhibition is enabled, then user request has to be ignored depending on inhibition type and
    * PNC state */
    ...
    * 3. PNC is in No communication or prepare sleep state
    ...
    if((COMM_GET_PREVENT_WAKEUP((globalRamPtr_pst->ComM_EcuGroupClassification_u8))) &&
        ...
        (((pncRamPtr_pst->PncState_en) == COMM_PNC_NO_COMMUNICATION) ||
                ((pncRamPtr_pst->PncState_en) == COMM_PNC_PREPARE_SLEEP)))
    {
    ...
    * indication from Nm. These requests are processed in the respective Pnc main functions.*/
    ...
    PncEventStruct_pst->activeRequestState_b = userRequestState_b;
    ...
    /* Evaluate if Synchronization Shutdown Request exists*/
    if((pncRamPtr_pst->PncSynchronizeShutdownRequest_b) != FALSE)
    {
        PncEventStruct_pst->synchronizeShutdownRequest_b = TRUE;
        /* Reset the Synchronization Request Flag*/
        pncRamPtr_pst->PncSynchronizeShutdownRequest_b = FALSE;
    }
                                                      ...
                                                      ComM_PncModeType                  previousPncState_en)
{
    ...
    if(previousPncState_en != COMM_PNC_REQUESTED)
    {
        ...
        /* BswM_ComM_CurrentPNCMode shall be called with COMM_PNC_REQUESTED_WITH_WAKEUP_REQUEST when entering
         * PNC sub state COMM_PNC_REQUESTED from COMM_PNC_NO_COM or COMM_PNC_PREPARE_SLEEP,
         * and ComMPncWakeupSleepRequestEnabled of this PNC is set to TRUE */
        if(((previousPncState_en == COMM_PNC_NO_COMMUNICATION) || (previousPncState_en == COMM_PNC_PREPARE_SLEEP)) &&
            (pncConfigPtr_pcst->ComMPncWakeupSleepRequestEnabled_b == TRUE))
        {
            /* Notify BswM regarding state change with state as COMM_PNC_REQUESTED_WITH_WAKEUP_REQUEST */
            BswM_ComM_CurrentPNCMode(PncIndex_tu8,COMM_PNC_REQUESTED_WITH_WAKEUP_REQUEST);
        }
        else
        {
            /* Notify BswM regarding state change */
        ...
        * channel for a GWType "active".
        * Only then the channel is requested for full communication for GWType "passive"
    ...
    else
    {
                    ...
                    if ((channelConfigPtr_pcst->PncGwType_en == COMM_GATEWAY_TYPE_PASSIVE))
                    {
                        if((channelConfigPtr_pcst->TxSigId_u16)!= COMM_COMSIGNAL_INVALID_ID)
                        {
                            ...
                            * Hence increment the PncRequestCtr_u8 value for this channel*/
                            ComM_Prv_PncToPassiveChannelInterface(channelRamPtr_pst,
                                                                   PncEventStruct_pst->activeERAState_b,
                            ...
                            if(PncEventStruct_pst->activeERAState_b != FALSE)
                            {
                                ...
                                /* MR12 DIR 1.1 VIOLATION: Com_SendSignal's SignalDataPtr return value should be void
                                * as per Autosar guidelines. */
                                (void)Com_SendSignal(channelConfigPtr_pcst->TxSigId_u16,
                                        &(channelRamPtr_pst->PnTxSigVal_u8[0]));
                            }
                            else
                            {
                                SchM_Enter_ComM_PNCTxSigNoNest();
                                ...
                                * as per Autosar guidelines. */
                                ...
                                (void)Com_SendSignal(channelConfigPtr_pcst->TxSigId_u16,
                                        (channelRamPtr_pst->PnTxSigVal_u8));
                                #endif  /*  (COMM_ZERO_PNC_VECTOR_AVOIDANCE == STD_OFF)  */
                            }
        ...
        else
        {
            ...
            #if (COMM_SYNCHRONIZED_PNC_SHUTDOWN != STD_OFF)
            /* No Active Request Exist then For INTERMEDIATE and TOP LEVEL Coordinator
            * Request Nm for Synchronize Shutdown
            */
            if((PncEventStruct_pst->synchronizeShutdownRequest_b != FALSE) ||
                  (pncConfigPtr_pcst->PncCoordinatorType_en == COMM_TOP_LEVEL_PNC))
            {
                for (loopCtr_u8=0u; loopCtr_u8 < pncConfigPtr_pcst->NumChannels_u8; loopCtr_u8++)
                {
                    channelId_u8 = pncConfigPtr_pcst->Channels_pcu8[loopCtr_u8];
                    channelConfigPtr_pcst = &(COMM_GET_CHANNEL_LIST[channelId_u8]);
                    ...
                    {
                        (void)Nm_RequestSynchronizedPncShutdown(channelConfigPtr_pcst->ComMChannelId_u8,
                                pncConfigPtr_pcst->PncId_u8);
                    }
                                                       ...
                                                       ComM_PncModeType                 previousPncState_en)
{
    ...
    if(previousPncState_en != COMM_PNC_READY_SLEEP)
    {
        ...
        * if ComM0PncVectorAvoidance is set to true in configuration. Or else the PncRequestCtr_u8 counter
        * will under flow*/
        ComM_Prv_PncToChannelInterface(PncIndex_tu8,COMM_PNC_READY_SLEEP,previousPncState_en,
                ActiveERAStateOldStatus_b);
        ...
        * call Com_SendSignal() with '0' for all Com signals assigned to this PN */
        if(previousPncState_en == COMM_PNC_REQUESTED)
        {
            /* For all ComM channels of this PNC,call Com_SendSignal() with '0' as this PN is not requested actively
            * anymore  */
            for(loopCtr_u8=0; loopCtr_u8 < pncConfigPtr_pcst->NumChannels_u8; loopCtr_u8++)
            {
                channelId_u8 = pncConfigPtr_pcst->Channels_pcu8[loopCtr_u8];

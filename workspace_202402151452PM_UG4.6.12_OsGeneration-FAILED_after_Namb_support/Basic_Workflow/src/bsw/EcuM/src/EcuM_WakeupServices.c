...
void EcuM_SetWakeupEvent( EcuM_WakeupSourceType sources )
{
    ...
    uint32 EcuM_newWkupEvents_u32 = ECUM_NO_WKUP_SRC;

    // internal variable used to hold the wakeup validation timeout
    uint16 EcuM_WkpSrcValidationTimeoutCtr_u16 = ECUM_TIMEOUT_U16;
    ...
    else
    {
        ...
        * associated to the selected sleep mode.*/
        ...
        if ( EcuM_newWkupEvents_u32 != ECUM_NO_WKUP_SRC )
        {
            ...
            for( ctrLoop_u8 = 0;ctrLoop_u8 < ECUM_NUM_OF_WKP_SOURCES;ctrLoop_u8++ )
            {
...

                {
                    ...
                    EcuM_Cfg_CheckWakeupTimer_ast[ctrLoop_u8].CheckWakeupTimeout = ECUM_TIMEOUT_U16;
        ...
        /*Update the Global variable EcuM_Prv_dataClrWkpEventsInd_u32 with the new event reported*/
        if ((EcuM_Prv_dataClrWkpEventsInd_u32 & EcuM_newWkupEvents_u32 ) != ECUM_NO_WKUP_SRC)
        {

            EcuM_Prv_dataClrWkpEventsInd_u32 ^= EcuM_Prv_dataClrWkpEventsInd_u32 & EcuM_newWkupEvents_u32;

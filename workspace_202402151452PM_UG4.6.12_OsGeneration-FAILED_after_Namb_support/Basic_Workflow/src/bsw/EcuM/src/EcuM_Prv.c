...
EcuM_ShutdownCauseType EcuM_Prv_dataSelectedShutdownCause_u8 = ECUM_SHUTDOWNCAUSE_DEFAULT_U8;
...
void EcuM_Prv_WakeupIndication( EcuM_WakeupSourceType dataPendingWakeupEvents_u32 )
{
    ...
    if (EcuM_Prv_dataValWkpEventsInd_u32 !=ECUM_NO_WKUP_SRC)
    {
        ...
        for(cntrLoopCtr_u8 = 0;cntrLoopCtr_u8 < ECUM_NUM_OF_WKP_SOURCES ; cntrLoopCtr_u8++)
        {
            if (((EcuM_Cfg_idxWakeupSourcesPC_au32 [cntrLoopCtr_u8].WakeupSourceId & EcuM_Prv_dataValWkpEventsInd_u32)
                    != ECUM_NO_WKUP_SRC) &&
                    (EcuM_Cfg_idxWakeupSourcesPC_au32 [cntrLoopCtr_u8].nbrOfComChannelPresent > 0))
            {
                for ( ctrLoopChlNotify_u8 = 0; ctrLoopChlNotify_u8 <  EcuM_Cfg_idxWakeupSourcesPC_au32[cntrLoopCtr_u8].nbrOfComChannelPresent; ctrLoopChlNotify_u8++)
                {
                    /*Give ComM Indication*/
                    ComM_EcuM_WakeUpIndication (EcuM_Cfg_idxWakeupSourcesPC_au32[cntrLoopCtr_u8].ComChannelReferance[ctrLoopChlNotify_u8]);
                }
            }
    ...
    if(EcuM_Prv_dataClrWkpEventsInd_u32 != ECUM_NO_WKUP_SRC)
    {
...
void EcuM_Prv_DecValidationCtr( void )
{
    ...
    if (EcuM_Prv_WkpSrcValidationTimeoutCtr_u16 > ECUM_TIMEOUT_U16)
    {
        SchM_Enter_EcuM();  /*Enter Critical Section*/
...
void EcuM_Prv_GoSleepSequence(uint16 dataSleepModeHandle_u16)
    ...
    EcuM_WakeupSourceType EcuM_dataPendingWakeupEvents_u32 = ECUM_NO_WKUP_SRC;
    EcuM_WakeupSourceType EcuM_dataValidatedWakeupEvents_u32 = ECUM_NO_WKUP_SRC;
...
static void EcuM_Prv_HaltingSequence(uint16 dataSleepModeHandle_u16)
    ...
    {
        if( ECUM_HASH_FAILURE_U8 == EcuM_CheckRamHash())
        {
...
void EcuM_Prv_WakeupRestartSequence ( uint16 dataSleepModeHandle_u16 )
  ...
  EcuM_Prv_McuSetMode( ECUM_MCU_MODE_ZERO_U8, flgOperation_en);
    ...
    if (flgOperation_en == ECUM_PRV_CLEAR_SLEEP_E)
    {
        ...
        for( ctrLoop1_u8 = 0;ctrLoop1_u8 < ECUM_NUM_OF_WKP_SOURCES;ctrLoop1_u8++ )
        {
            ...
            else if(ECUM_TIMER_STARTED_E == EcuM_Cfg_CheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupStatus)
            {
                ...
                EcuM_Cfg_CheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupTimeout = EcuM_Cfg_CheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupTimeout + ECUM_TIMER_INCREMENT_U16;
                SchM_Exit_EcuM();        /*Leave Critical Section*/
            }
            else
            {
...
uint32 EcuM_Prv_ComMWakeupHandling (EcuM_WakeupSourceType dataPendingWakeupEvents)
    ...
    uint32 EcuM_CommchlWkupEvents_u32 = ECUM_NO_WKUP_SRC;
    uint8 ctrLoop_u8 = 0;

    /* Switch to either enable or disable call to COMM functions */
    #if( ECUM_INCLUDE_COMM == STD_ON )
    uint8 ctrLoopPNCNotify_u8 = 0;
    uint8 ctrLoopChlNotify_u8 = 0;
    #endif  /* ECUM_INCLUDE_COMM */
    ...
    for( ctrLoop_u8 = 0;ctrLoop_u8 < ECUM_NUM_OF_WKP_SOURCES;ctrLoop_u8++ )
    {
        ...
        if ( ( ( dataPendingWakeupEvents & EcuM_Cfg_idxWakeupSourcesPC_au32[ctrLoop_u8].WakeupSourceId ) != ECUM_NO_WKUP_SRC ) \
                && ( EcuM_Cfg_idxWakeupSourcesPC_au32[ctrLoop_u8].nbrOfComChannelPresent > 0) )
        {
            /* Switch to either enable or disable call to COMM functions */
            #if( ECUM_INCLUDE_COMM == STD_ON )
            for ( ctrLoopChlNotify_u8 = 0; ctrLoopChlNotify_u8 <  EcuM_Cfg_idxWakeupSourcesPC_au32[ctrLoop_u8].nbrOfComChannelPresent; ctrLoopChlNotify_u8++)
            {
                    ComM_EcuM_WakeUpIndication ( EcuM_Cfg_idxWakeupSourcesPC_au32[ctrLoop_u8].ComChannelReferance[ctrLoopChlNotify_u8] );
            }
            #endif  /* ECUM_INCLUDE_COMM */

...
void ComM_DeInit(void)
{
    ...
    if (globalRamPtr_pst->ComM_InitStatus_en != COMM_INIT)
    {
        /* Report DET with ComM not initialised status*/
        COMM_DET_REPORT_ERROR(COMM_DEINIT_APIID, COMM_E_NOT_INITED);
        return;
    }
    else
    {
    ...
    for (ChannelIndexCtr_u8 = C_ZERO ; ChannelIndexCtr_u8 < COMM_NO_OF_CHANNELS ; ChannelIndexCtr_u8++)
    {
        ...
        /* MR12 RULE 14.2 VIOLATION: The variable incremented in the third expression of this 'for' statement
        * is actually the variable identified as the 'loop control variable.
        * Hence it is an incorrect warning reported by Misra */

        for (ChannelId_u8 = C_ZERO; ChannelId_u8 < COMM_NO_OF_CHANNELS ;++ChannelId_u8)
        {
            /*MR12 RULE 10.3 VIOLATION:  An expression of 'essentially Boolean' type (_Bool) is being converted to
            ...
            * This is a Incorrect Misra Warning. This is taken care in review.*/
            ...
            ComM_Prv_UpdateNvMInhibitionStatus(ChannelId_u8, COMM_GET_PREVENT_WAKEUP(channelRamPtr_pst->
                                               InhibitionReqStatus_u8), ComM_NvMblock.ComMNoWakeup_u8);
        }
        ...
        {
            COMM_DET_REPORT_ERROR(COMM_NVM_REFERENCE_BLOCK, COMM_E_NVM_WRITE_FAILED);
            return;
        }
...

{
    ...
    bitPosition_u8 = (ChannelId_u8) % 8u ;

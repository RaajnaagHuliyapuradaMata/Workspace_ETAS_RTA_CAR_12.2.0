                                                                ...
                                                                uint8* Buffer_pu8)
{
    ...
    if (NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo, NVM_PRV_BLOCK_FLAG_CNTR_WRITE))
    {
        SizeWriteCntr_u16 = sizeof(NvM_Prv_cntrWrite_auo[idBlock_uo]);

        // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3290] If user requests a write job prior to a read request

...
RBA_MEMLIB_STATIC boolean NvM_Prv_BlockSplit_Invalidate_IsEnabled(NvM_Prv_JobData_tst const* JobData_pcst);

RBA_MEMLIB_STATIC NvM_Prv_Job_State_tpfct NvM_Prv_BlockSplit_Invalidate_GetStateFct(NvM_Prv_stJob_ten stJob_en);

RBA_MEMLIB_STATIC NvM_Prv_idJobStateMachine_ten NvM_Prv_BlockSplit_Invalidate_DoSubBlocks(NvM_Prv_stJob_ten* stJob_pen,
...
RBA_MEMLIB_STATIC_CONST NvM_Prv_Job_StateMachine_Data_tst NvM_Prv_BlockSplit_Invalidate_Data_cst =
{
        &NvM_Prv_BlockSplit_Invalidate_GetStateFct,
        &NvM_Prv_BlockSplit_Invalidate_IsEnabled,
...
NvM_Prv_Job_StateMachine_Data_tst const* NvM_Prv_BlockSplit_Invalidate_GetData(void)
{
#if (NVM_BLOCK_COMPRESSION_USED == STD_ON)
    return &NvM_Prv_BlockSplit_Invalidate_Data_cst;
#else
    return NULL_PTR;
#endif
}
...
RBA_MEMLIB_STATIC boolean NvM_Prv_BlockSplit_Invalidate_IsEnabled(NvM_Prv_JobData_tst const* JobData_pcst)
{
    // Invalidate job for split blocks is enabled only for configured split blocks if master-block is in use
...
RBA_MEMLIB_STATIC NvM_Prv_Job_State_tpfct NvM_Prv_BlockSplit_Invalidate_GetStateFct(NvM_Prv_stJob_ten stJob_en)
{
    NvM_Prv_Job_State_tpfct Invalidate_State_pfct = NULL_PTR;

    switch (stJob_en)
    {
        case NvM_Prv_stJob_BlockSplit_Invalidate_DoSubBlocks_e:
            Invalidate_State_pfct = &NvM_Prv_BlockSplit_Invalidate_DoSubBlocks;
        break;

        default:
            Invalidate_State_pfct = NULL_PTR;
        break;
    }

    return Invalidate_State_pfct;
}
...
RBA_MEMLIB_STATIC NvM_Prv_idJobStateMachine_ten NvM_Prv_BlockSplit_Invalidate_DoSubBlocks(NvM_Prv_stJob_ten* stJob_pen,
                                                                                          ...
                                                                                          NvM_Prv_JobData_tst const* JobData_pcst)
{
    NvM_Prv_idJobStateMachine_ten idJobStateMachineNext_en = NvM_Prv_idJobStateMachine_DoBlockSplit_Invalidate_e;
    ...
    idxSubBlock_u8 = NvM_Prv_BlockSplit_GetIdxSubBlockForInvalidation(idxSubBlock_u8,
                                                                      nrSubBlocks_u8,
                                                                      ArrayIdxSubBlockCurrent_pu8,
                                                                      ArrayIdxSubBlockNew_pu8);
    ...
    if (idxSubBlock_u8 < nrSubBlocks_u8)
    {
        ...
        // start DoMemIf_Invalidate-StateMachine for current sub-block for invalidation
        *stJob_pen = NvM_Prv_stJob_BlockSplit_Invalidate_DoSubBlocks_e;
    ...
    {
        // all required sub-blocks invalidated -> complete job
        *stJob_pen = NvM_Prv_stJob_BlockSplit_Invalidate_Completed_e;
        JobResult_pst->Result_en = NvM_Prv_JobResult_Succeeded_e;

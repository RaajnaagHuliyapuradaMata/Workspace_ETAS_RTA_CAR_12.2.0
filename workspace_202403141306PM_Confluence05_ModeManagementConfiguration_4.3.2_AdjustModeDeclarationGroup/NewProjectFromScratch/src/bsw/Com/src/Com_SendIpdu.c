...
LOCAL_INLINE void Com_Prv_LoadMinimumDelayTime(Com_TxIpduRam_tpst txIpduRamPtr_pst,
...
void Com_Prv_SendIpdu(PduIdType idTxPdu_uo, Com_SendIpduInfo_tst sendIpduFlag_st)
{
    ...
    * Com_SendSignalWithMetaData()              if COM_METADATA_SUPPORT macro is enabled
    * Com_SendSignalGroupWithMetaData()         if COM_METADATA_SUPPORT and COM_TX_SIGNALGROUP macros are enabled
    * Com_SendSignalGroupArrayWithMetaData()    if COM_METADATA_SUPPORT and COM_TX_SIGNALGROUP_ARRAY macros are enabled
    */
...
#if !defined(COM_TXPDU_DEFERREDPROCESSING)
# if (defined (COM_SIGNALGATEWAY) || defined (COM_SIGNALGROUPGATEWAY))
        Com_SetRamValue(TXIPDU,_GWIPDU_TXREQ,txIpduRamPtr_pst->txFlags_u16,COM_FALSE);
# endif
#endif
    ...
    if (sendIpdu_b)
    {
        ...
        if (Com_GetValue(TXIPDU,_IS_METADATAPDU,txIpduConstPtr_pcst->txIPduFields_u16))
        {
            ...
            {
                /* Restore default metadata in case request MetadataPtr is NULL_PTR */
                Com_ByteCopy( (metaDataInfoPtr_pcst->txMetaDataValue_pau8),
            ...
            Com_Get_SchM_Exit_Com_TxIPduMetaData
        }
        ...
        */
        if ((txIpduConstPtr_pcst->callOut_pfct != NULL_PTR) && (isTransmitCallout_b))
        {
            ...
            ipduCalloutStatus_b =
                    txIpduConstPtr_pcst->callOut_pfct((PduIdType)COM_GET_TXIPDU_HANDLEID(idTxPdu_uo), &txIPduInfo_st);
        ...
        if (Com_GetValue(TXIPDU,_ISLARGEDATAPDU,txIpduConstPtr_pcst->txIPduFields_u16))
        {
            ...
            * PduR_ComTransmit, the AUTOSAR COM module shall not modify the data of its internal transmit buffer of
            * this I-PDU. */
            Com_SetRamValue(TXIPDU,_LARGEDATAINPROG,txIpduRamPtr_pst->txFlags_u16,COM_TRUE);
            txIpduRamPtr_pst->txTpCopiedLength_uo = COM_ZERO;
        }
...
LOCAL_INLINE boolean Com_Prv_LoadPeriodicModeInfo(PduIdType idTxPdu_uo, Com_SendIpduInfo_tst sendIpduFlag_st)
{
    ...
    txIpduRamPtr_pst = COM_GET_TX_IPDU_CONSTDATA(idTxPdu_uo)->ipduRam_pst;
    ...
    */
    ...
    switch (Com_GetRamValue(TXIPDU,_LATESTMODE,txIpduRamPtr_pst->transMode_u8))
    {
        case COM_TXMODE_PERIODIC:
        {
            ...
            {
                /*TRACE[SWS_Com_00881] When the transmission mode of an I-PDU is explicitly set by
                * Com_SwitchIpduTxMode, the AUTOSAR COM shall defer the cyclic transmissions of this I-PDU by
                * ComTxModeTimeOffset.*/
                txIpduRamPtr_pst->cntrTimePeriod_u16 = COM_GET_TXIPDU_TIMEOFFSET(idTxPdu_uo,
        ...
        case COM_TXMODE_MIXED:
        {
            ...
            * periodic mode immediately, respecting MDT. */
            ...
            else
            {
                ...
                txIpduRamPtr_pst->cntrTimePeriod_u16 = COM_GET_TXIPDU_TIMEPERIOD(idTxPdu_uo,
                                                                            txIpduRamPtr_pst->currentTxModePtr_pcst);
...
LOCAL_INLINE boolean Com_Prv_LoadEventModeInfo(PduIdType idTxPdu_uo, Com_SendIpduInfo_tst sendIpduFlag_st)
{
    ...
    if (sendIpduFlag_st.ignoreRepetitions_u1 == COM_RESET)
    {
        ...
        * then send the IPDU synchronous with Com_MainFunctionTx, respecting MDT */
        txIpduRamPtr_pst->cntrRepetitions_u8 = COM_GET_TXIPDU_NUMOFREPETITION(idTxPdu_uo,
                                                                            txIpduRamPtr_pst->currentTxModePtr_pcst);
    }
    else
    {
...
# if defined (COM_SIGNALGATEWAY) || defined (COM_SIGNALGROUPGATEWAY)
            if (sendIpduFlag_st.prepareGwIpduTx_u1 == COM_SET)
            {
                /* Invoked from MainFunctionTx, to trigger Gw IPdu, but to respect below requirement,
                 * flag the transmission request now, and transmit later in the same call of MainFunctionTx.
                * TRACE[SWS_Com_00478] The AUTOSAR COM module shall send out an I-PDU at most
                * once within one call of Com_MainFunctionTx. */
                Com_SetRamValue(TXIPDU,_GWIPDU_TXREQ,txIpduRamPtr_pst->txFlags_u16,COM_TRUE);
            }
            else
...
LOCAL_INLINE PduLengthType Com_Prv_CalculateTxPduLength(PduIdType idTxPdu_uo)
{
...
#if defined(COM_TX_DYNAMIC_SIGNAL_SUPPORT) || defined(COM_TX_DYNAMIC_GROUPSIGNAL_SUPPORT) || defined(COM_TX_TP_IPDUTYPE)
    Com_TxIpduCfg_tpcst         txIpduConstPtr_pcst;
    Com_TxIpduRam_tpst          txIpduRamPtr_pst;
    ...
    * that shall be used in Com_CopyTxData() */
    txIpduRamPtr_pst->txTpLength_uo = txPduLength_uo;
#endif
    ...
    * via PduR_ComTransmit. */
    ...
    if (ipduTransStatus_u8 == E_OK)
    {
        ...
        * after this I-PDU was sent out via PduR_ComTransmit and PduR_ComTransmit returned E_OK. */
        ...
        if (Com_GetValue(TXIPDU,_CLRUPDBIT,txIpduConstPtr_pcst->txIPduFields_u16) == COM_CLRUPDATEBIT_TRANSMIT)
        {
...
LOCAL_INLINE void Com_Prv_TxIpduIncrementCounter(PduIdType idTxPdu_uo)
{
    ...
    * with respect to counter wrap around, directly before the I-PDU is sent out by PduR_ComTransmit. */
    ...
    counterValue_u8 = (uint8)((uint16)txIpduConstPtr_pcst->ipduRam_pst->txIpduCntrValue_u8 + COM_ONE);
    ...
    txIpduConstPtr_pcst->ipduRam_pst->txIpduCntrValue_u8 = counterValue_u8;
...
LOCAL_INLINE void Com_Prv_TxIpduDecrementCounter(PduIdType idTxPdu_uo)
{
    ...
    counterValue_u8 = txIpduConstPtr_pcst->ipduRam_pst->txIpduCntrValue_u8;

      ...
      defined(COM_TxIPduCallOuts))
        ...
        * No MDT monitoring is required */
        ...
        if (Com_GetValue(TXIPDU,_IS_METADATAPDU,txIpduConstPtr_pcst->txIPduFields_u16))
        {
            ...
            {
                /* Copy the given MetaData into Tx PduInfoType in case request MetadataPtr is not NULL_PTR */
                if(pduInfoPtr_pst->MetaDataPtr != NULL_PTR)
                {
                    Com_ByteCopy(pduInfoPtr_pst->MetaDataPtr, metaDataInfoPtr_pcst->txMetaDataValue_pau8, metaDataInfoPtr_pcst->txMetaDataLength_u8);
                }
            ...
            {
                /* Copy the given default MetaData into PduInfoType in case request MetadataPtr is not NULL_PTR */
                if(pduInfoPtr_pst->MetaDataPtr != NULL_PTR)
                {
                    Com_ByteCopy( (pduInfoPtr_pst->MetaDataPtr),
...
#if (defined(COM_TxSigUpdateBit) || defined(COM_TxSigGrpUpdateBit))
        if (Com_GetValue(TXIPDU,_CLRUPDBIT,constByteValue_u16) == COM_CLRUPDATEBIT_TRIGGERTRANSMIT)
        {
        ...
        if (status_u8 != E_NOT_OK)
        {
            if (Com_GetValue(TXIPDU,_ISCALLOUTFRMTRIGTRANS,constByteValue_u16))
            {
                ...
                * return value from the ComIPduTriggerTransmitCallout. */
                (void)txIpduConstPtr_pcst->callOut_pfct((PduIdType)COM_GET_TXIPDU_HANDLEID(idTxPdu_uo), pduInfoPtr_pst);
            }
        }
#endif
    }

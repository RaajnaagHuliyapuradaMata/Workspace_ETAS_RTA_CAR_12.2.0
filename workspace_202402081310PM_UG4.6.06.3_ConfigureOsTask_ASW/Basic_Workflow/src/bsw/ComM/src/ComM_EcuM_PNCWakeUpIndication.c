...
void ComM_EcuM_PNCWakeUpIndication(PNCHandleType PNCid)
{
    ...
    if (ComM_GlobalStruct.ComM_InitStatus_en != COMM_INIT)
    {
        ...
        COMM_DET_REPORT_ERROR(COMM_ECUM_PNCWAKEUP_INDICATION, COMM_E_NOT_INITED);
        return;
    }
    ...
    if (PNCid >= COMM_NO_OF_PNCS)
    {
        ...
        COMM_DET_REPORT_ERROR(COMM_ECUM_PNCWAKEUP_INDICATION, COMM_E_WRONG_PARAMETERS);
        return;
    }
    /*********************************************DET END************************************************/
    ...
    * it will not always be true. */
    if(COMM_GET_FEATURE_PNC_ENABLED != FALSE)
    {
        ...
        /* Lock required for consistent update of PassiveRequestState_u8 in API's and main function along with channel
        * state */
        #if (COMM_SYNCHRONOUS_WAKE_UP != STD_ON )
    ...
    * Hence report to DET */
    COMM_DET_REPORT_ERROR(COMM_ECUM_PNCWAKEUP_INDICATION, COMM_E_ERROR_IN_PROV_SERVICE);

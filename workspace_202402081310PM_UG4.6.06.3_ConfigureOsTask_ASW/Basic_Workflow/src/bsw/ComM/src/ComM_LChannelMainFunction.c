...
LOCAL_INLINE void ComM_Prv_RequestBusSm(ComM_ChannelVarType_tst *    channelRamPtr_pst,
                                        const ComM_ChannelType_tst * channelConfigPtr_pcst,
                                        ComM_ModeType                channelMode_tu8);
...
void ComM_Prv_ChannelMainFunction(NetworkHandleType ChannelIndex_tu8)
{
    ...
    if (ComM_GlobalStruct.ComM_InitStatus_en != COMM_INIT)
    {
        ...
        COMM_DET_REPORT_ERROR(COMM_MAINFNC_APIID, COMM_E_NOT_INITED);
        return;
    }
    ...
    * 3. channel is in NO COMMUNICATION or SILENT_COMMUNICATION mode */
    if((COMM_GET_PREVENT_WAKEUP(globalRamPtr_pst->ComM_EcuGroupClassification_u8)))
    {
        if((COMM_GET_PREVENT_WAKEUP(channelRamPtr_pst->InhibitionReqStatus_u8)) &&
                ((channelRamPtr_pst->ChannelMode_u8 == COMM_NO_COMMUNICATION) || (channelRamPtr_pst->ChannelMode_u8 ==
    ...
    do
    {
        ...
        * 2. Poll and Retry : Check BusSm state and Request BusSm to go to respective mode */
        ...
        switch (channelState_en)
        {
            ...
            {
                channelState_en = ComM_Prv_NoComPending_StateHandling(requestStructPtr_pst,
                                        channelRamPtr_pst, previousstate_en);
            }
            break;
            ...
            {
                channelState_en = ComM_Prv_SilentCom_StateHandling(requestStructPtr_pst, channelRamPtr_pst,
                        channelConfigPtr_pcst, previousstate_en);
            }
            break;
...
/* MR12 RULE 8.13 VIOLATION: The requestStructPtr_pst pointer variables are modified in main function.
 * Hence it cant be P2CONST type */
/* HIS METRIC PATH, V(G) VIOLATION IN ComM_Prv_FullComNetworkReq_StateHandling:
   The function involves optimal  COMM_FULL_COM_NETWORK_REQUESTED state handlings through simple if and switch cases.
                                                                     ...
                                                                     ComM_StateType               previousstate_en)
{
        ...
        switch (channelConfigPtr_pcst->ComMNmVariant_en)
        {
            ...
            * otherwise, transition to ready sleep has to be made */
            case (FULL):
            {
                ...
                {
                    /* As there is no active request,proceed directly to Ready sleep as there is no
            ...
            /* For ComMNmVariant : PASSIVE, there cannot be active request,
             * hence change to Ready sleep directly */
            case (PASSIVE):
            {
            ...
            case SLAVE_ACTIVE:
            {
                if(requestStructPtr_pst->ActiveRequest_b)
                {
                    /* As there is active request,COMM_FULL_COM_NETWORK_REQUESTED state has to be retained  */
                }
                else
                {
            ...
            /* For ComMNmVariant: SLAVE_PASSIVE, there cannot be active request,
             * hence change to Ready sleep directly */
            case SLAVE_PASSIVE:
            {
                /* As the Nm variant Slave passive, proceed directly to Ready sleep state */
                channelState_en = COMM_FULL_COM_READY_SLEEP;
            }
            break;
    ...
    else
    {
        switch (channelConfigPtr_pcst->ComMNmVariant_en)
        {
            ...
            * */
                ...
                else if(requestStructPtr_pst->ActiveRequest_b != FALSE)
                {
                    ...
                    * therefore it will not always be true. */
                    if(COMM_GET_FEATURE_PNC_ENABLED)
                    {
                        ...
                        {
                            (void)Nm_NetworkRequest(channelConfigPtr_pcst->ComMChannelId_u8);
                            /* Lock is not required here as the variable is set in pnc main function and
                            * comm mainfunction clears it and it is scheduled after pnc */
                            channelRamPtr_pst ->NmNetworkRequestStatus_b = FALSE;
                        }
                    }
                    #endif /* #if(COMM_PNC_ENABLED != STD_OFF)  */

                    /* Request BusSm with Channel Mode COMM_FULL_COMMUNICATION_WITH_WAKEUP_REQUEST,
                     * if ComMFullComWakeupRequestStatus_b is TRUE */
                    if(channelRamPtr_pst->ComMFullComWakeupRequestStatus_b == TRUE)
                    {
                        /* Request BusSm */
                        ComM_Prv_RequestBusSm(channelRamPtr_pst, channelConfigPtr_pcst,
            ...
            {
                /* Request BusSm with Channel Mode COMM_FULL_COMMUNICATION_WITH_WAKEUP_REQUEST,
                 * if there is active request present and ComMFullComWakeupRequestStatus_b is TRUE */
                if((requestStructPtr_pst->ActiveRequest_b == TRUE) &&
                ...
                /* If DCM does not indicate diagnostic request and if Communication Limitation is requested,
                * cancel the timer TMinFullComModeDuration */
                #if (COMM_LIMIT_TO_NOCOM_ENABLED != STD_OFF)
                ...
                if((channelRamPtr_pst->MinFullComTimeoutCtr_u16) == C_ZERO)
                {
                    ...
                    {
                        /* Timer expired/cancelled and there is no active request, so proceed to Ready sleep state */
                        channelState_en = COMM_FULL_COM_READY_SLEEP;
                    }
                }
            }
            ...
            case SLAVE_ACTIVE:
            {
                 /* For nmVariant SLAVE_ACTIVE , priority of requests is
                 *             1. BusSM sleep indication
                 *             2. Active request
                 *             proceeed to  ready sleep
                 *  Process these requests according to above priority */
                if((channelRamPtr_pst->BusSMSleepIndicationStatus_b) != FALSE)
                {
                    channelState_en = COMM_NO_COM_NO_PENDING_REQUEST;
                }
                else if(requestStructPtr_pst->ActiveRequest_b == FALSE)
                ...
                {
                    /* As there is active request,COMM_FULL_COM_NETWORK_REQUESTED
                                             * state has to be retained  */
                }
    ...
    {
        /* Clear the busNm and busSM sleep indication flag as it is already processed.
           If NmBusSleepIndicationStatus_b and BusSMBusSleepIndicationStatus_b is set and processed, then channelState_en will be
        ...
        channelRamPtr_pst->BusSMSleepIndicationStatus_b = FALSE;
    }
...

{
    ...
    if (previousstate_en != COMM_FULL_COM_READY_SLEEP )
    {
        ...
        if(previousstate_en == COMM_FULL_COM_NETWORK_REQUESTED)
        {
            ...
            {
                #if(COMM_NM_SUPPORT != STD_OFF)
                /* Entry to ready sleep state from Network Requested and from Silent Communication has to be
                * differentiated On Entry to Ready sleep from Network requested, Nm api's needs to be called if
                * Nm is present but on entry to Ready sleep from Silent Com, No Nm api's needs to be called,
                * as silent com -> Ready sleep is only triggered by Nm (Nm_NetworkMode) */
                #if(COMM_NM_VARIANT == COMM_NM_VARIANT_FULL)
                case FULL :
                {
                    ...
                    * will be >= 2*/
                    /* There is two state transitions from no com to full com in the same main function when it is
                    * passive request for full_com*/
                ...
                * hence entry need not be differentiated*/
                case LIGHT:
                {
                    ...
                    /* Check if NmLightTimeOut timer expired i.e. check if value configured for NmLightTimeout
                    * timer is zero, then proceed directly to no_com_no_pending state*/
                ...
                case NONE:
                {
                    ...
                    {
                        /*For Bus types other than internal,if NM variant is NONE channel will remain
                        * in Ready sleep state.
                ...
                case SLAVE_PASSIVE:
                {
                     /* As no user request exists in COMM_FULL_COM_READY_SLEEP */
                }
                break;
    ...
    else
    {
        switch (channelConfigPtr_pcst->ComMNmVariant_en)
        {
            ...
            * These requests are processed according to above priority*/
            ...
            case SLAVE_PASSIVE:
            {
                 /* For nmVariant SLAVE_PASSIVE , ComM channel state machine shall switch to state
                  * COMM_NO_COMMUNICATION when Bus State Manager module indicates ComM_BusSm_BusSleepMode() */
                if((channelRamPtr_pst->BusSMSleepIndicationStatus_b) != FALSE)
                {
                    channelState_en = COMM_NO_COM_NO_PENDING_REQUEST;
                }
    ...
    {
        SchM_Enter_ComM_ChannelNoNest();
        /* Since the flag NmPrepareBusSleepIndicationStatus can be set in COMM_FULL_COM_READY_SLEEP,
        * clear as the state is exited  */
        /*If this flag is not cleared then if the next state is n/w requested,
        * later channel  should switch to silent_com*/
        #if(COMM_NM_SUPPORT != STD_OFF)
        ...
        /* Clear the busSM sleep indication flag as it is already processed.
        If BusSMSleepIndicationStatus_b is set and processed, then channelState_en will be
        COMM_NO_COM_NO_PENDING_REQUEST.Hence the flag is cleared only in this condition  */
        channelRamPtr_pst->BusSMSleepIndicationStatus_b = FALSE;
                                        ...
                                        boolean                      entryToMode_b)
{
    ...
    if(requestBusSm_b)
    {
        /* If ComMFullComWakeupRequestStatus_b is TRUE, set Channel Mode to
         * COMM_FULL_COMMUNICATION_WITH_WAKEUP_REQUEST */
        if(channelRamPtr_pst->ComMFullComWakeupRequestStatus_b == TRUE)
        {
            channelMode_tu8 = COMM_FULL_COMMUNICATION_WITH_WAKEUP_REQUEST;
        }
        ComM_Prv_RequestBusSm(channelRamPtr_pst, channelConfigPtr_pcst, channelMode_tu8);
    }

}

/* ComM_Prv_RequestBusSm */
/**************************************************************************************
 * Function name    : ComM_Prv_RequestBusSm
 * Syntax           : ComM_Prv_RequestBusSm
 *                    (
 *                       ComM_ChannelVarType_tst *    channelRamPtr_pst,
                         const ComM_ChannelType_tst * channelConfigPtr_pcst,
                         ComM_ModeType channelMode_e
 *                     )
 * Description      : Internal function to request <Bus>Sm
 * Parameters       : const ComM_ChannelType_tst * channelConfigPtr_pcst,
                      ComM_ModeType channelMode_e
 * Return value     : void
 **************************************************************************************/
                                        ...
                                        ComM_ModeType                channelMode_tu8)
{
    ...
    /* RequestBusSm for mode change depending on Bus type. Possible Communication modes are,
    ...
    SchM_Enter_ComM_ChannelNoNest();
    if(channelMode_tu8 == COMM_FULL_COMMUNICATION_WITH_WAKEUP_REQUEST)
    {
        /* Clear ComMFullComWakeupRequestStatus_b flag once the request is processed */
        channelRamPtr_pst->ComMFullComWakeupRequestStatus_b = FALSE;
    }
    SchM_Exit_ComM_ChannelNoNest();

    if(RetValue == E_NOT_OK)
    {
        COMM_DET_REPORT_ERROR(COMM_MAINFNC_APIID, COMM_E_ERROR_IN_PROV_SERVICE);
        return;
    }
}

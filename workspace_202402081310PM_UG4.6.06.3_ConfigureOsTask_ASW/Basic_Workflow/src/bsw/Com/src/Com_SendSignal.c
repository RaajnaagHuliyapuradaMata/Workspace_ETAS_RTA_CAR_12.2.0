...
#ifdef COM_TX_TP_IPDUTYPE
        txIpduRamPtr_pst  =
                COM_GET_TX_IPDU_CONSTDATA( COM_GET_TXSIG_CONSTDATA( idSignal_u16 )->idComIPdu_uo )->ipduRam_pst;
...
{
#if defined(COM_TxSigUpdateBit) || defined(COM_EffectiveSigTOC)
    ...
    if (COM_SIG_TP_IS_TRIGGERED_ON_CHANGE(txSigTP_u8))
    {
        ...
        else if (COM_GET_OLDVALTOC(idTxMainFunc_uo,txSigConstPtr_pcst->idxOldValue_uo) !=
                                                                                    (Com_OldValTOC_tuo)txSigNewVal_uo)
        {
            isSigTriggered_u8 = COM_SET;
            COM_GET_OLDVALTOC(idTxMainFunc_uo,txSigConstPtr_pcst->idxOldValue_uo)= (Com_OldValTOC_tuo)txSigNewVal_uo;
        }
        else
        {
            /* Do nothing */
    ...
    * even for stopped I-PDUs. So the IPDU Group status is checked only after Signal packing is done. */
    if (Com_Prv_CheckTxIPduStatus((PduIdType)idIpdu_uo))
    {
        ...
        * to TMS calculation.
        */
        if (Com_GetValue(TXSIG,_FILTRALG,constTxSignalFields_u16) != COM_NOTCONFIGURED)
        {
        ...
        * so suppressing warning "Cast from a pointer to void to a pointer to object". */
        switch (type_u8)
        {
            case COM_SINT8:
            {
                ...
                txSigNewVal_uo = (uint32)(*(const sint8 * )signalDataPtr_pcv);
            }
            break;
            case COM_UINT8:
            {
            ...
            case COM_SINT16:
            {
                ...
                txSigNewVal_uo = (uint32)(*(const sint16 * )signalDataPtr_pcv);
            }
            break;
            case COM_UINT16:
            {
            ...
            case COM_SINT32:
            {
                ...
                txSigNewVal_uo = (uint32)(*(const sint32 * )signalDataPtr_pcv);
            }
            break;
            case COM_UINT32:
            {
            ...
            case COM_SINT64:
            {
                ...
                txSigNewVal_uo = (Com_SigMax_tuo)(*(const sint64 * )signalDataPtr_pcv);
            }
            break;
            case COM_UINT64:
            {
                /* Update the Src_Buf with the data i.e typecast the data to the uint64 variable*/
                txSigNewVal_uo = (Com_SigMax_tuo)(*(const uint64 * )signalDataPtr_pcv);
            }
            break;
        ...
        if (type_u8 == COM_FLOAT64)
        {
            ...
            * so suppressing warning "Cast from a pointer to void to a pointer to object". */
            ...
            /* Convert the float value into 32 bit stream and then pack the value */
            Com_Prv_PackFloatSignal(sigEndianess_u8, txSigConstPtr_pcst->bitPos_uo, txSigNewVal_f64,
    ...
    else
    {
        ...
        */
        ...
        if (COM_SIG_TP_IS_TRIGGERED_ON_CHANGE(txSigTP_u8))
        {
            ...
            * 2.Memcmp expects valid Type argument to be passed, but here signalDataPtr_pcv is of void type.
            * So, no functional impact.*/
            if ((COM_MEMCOMPARE((txIpduConstPtr_pcst->buffPtr_pu8 + byteNo_uo),signalDataPtr_pcv,
                                (uint32)txSigConstPtr_pcst->bitSize_uo)) != 0x00)
            {

                /* MR12 RULE 11.5 VIOLATION: signalDataPtr_pcv is defined as void pointer in the AUTOSAR specification,
                * so suppressing warning "Cast from a pointer to void to a pointer to object". */
                Com_ByteCopy((txIpduConstPtr_pcst->buffPtr_pu8 + byteNo_uo),(const uint8*)signalDataPtr_pcv,
                            (uint32)txSigConstPtr_pcst->bitSize_uo);

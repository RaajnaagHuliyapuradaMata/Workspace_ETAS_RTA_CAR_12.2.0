    ...
    const rba_CanDirectHWInfo_o * hw_info_po)
{
   ...
   /*Make a local copy of Device Mode. Try to get the CAN Drv controller state*/
    ...
    if (CAN_CS_STARTED == ((CanIf_Prv_ControllerState_ast[lControllerId_u8].Ctrl_Pdu_mode &CANIF_BIT_MASK_PDU_MODE)>>CANIF_Ctrl_BIT_SHIFT))
    {
...
#include "CanIf_MemMap.h"
void CANIF_CONTROLLERERRORPASSIVE_FNAME(uint8 ControllerId)
{
                                                    ...
                                                    uint32 * TxPduCanId)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NOT_OK((FALSE == CanIf_Prv_InitStatus_b), CANIF_RB_READ_TXPDU_CANID, CANIF_E_UNINIT)
    ...
    return RetVal;  /*Returns E_NOT_OK if TxPduCanId is not configured in large MetaDataItemLength case*/
}
        ...
        uint32 * RxPduCanId)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NOT_OK(CanIf_Prv_InitStatus_b == FALSE, CANIF_RB_READRXPDUCANID_SID,CANIF_E_UNINIT)
                                                    ...
                                                    )
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NOT_OK(CanIf_Prv_InitStatus_b == FALSE, CANIF_RETURN_TX_PDU_ID_SID,CANIF_E_UNINIT)
        ...
        PduIdType * CanIfRxPduIdPtr)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NOT_OK(CanIf_Prv_InitStatus_b == FALSE, CANIF_RETURN_RX_PDU_ID_SID,CANIF_E_UNINIT)
...
uint32 CanIf_ReadTxMsgId( uint16 msgHandleTx_u16 )
{
    ...
    if(CanIf_Prv_InitStatus_b == FALSE)
       {
       return ( (uint32)CanId );
       }
    lTxPduConfig_pst = CanIf_Prv_ConfigSet_tpst->CanIf_TxPduConfigPtr;
    ...
    return ( (uint32)CanId );   /*Returns invalid CanId(0xFFFFFFFF) if TxPduCanId is not configured in large MetaDataItemLength case*/
}
...
uint32 CanIf_ReadRxMsgId( uint16 msgHandleRx_u16 )
{
    ...
    /*[SWS_CANIF_00661] Det should be invoked if module is not initialized */
       CANIF_DET_REPORT_ERROR_INIT_NOT_OK(CanIf_Prv_InitStatus_b == FALSE, CANIF_READ_RX_MSG_ID_SID,CANIF_E_UNINIT)
...
Std_ReturnType CanIf_Rb_ChangeRxPduCanId(PduIdType CanRxPduId, Can_IdType CanId)
{
    ...
    /* If CAN Interface is uninitialized, report to DET and return E_NOT_OK */
     CANIF_DET_REPORT_ERROR_INIT_NOT_OK((FALSE == CanIf_Prv_InitStatus_b), CANIF_RB_CHANGERXPDUID_SID, CANIF_E_UNINIT)
   ...
   CANIF_DET_REPORT_ERROR_NOT_OK((CANIF_STATIC == (RxPduCfg_pcst->RxPduType)), CANIF_RB_CHANGERXPDUID_SID,CANIF_E_INVALID_RXPDUID)
    ...
    if( (((ControllerState_ps->Ctrl_Pdu_mode)&CANIF_BIT_MASK_PDU_MODE)>>CANIF_Ctrl_BIT_SHIFT) == CAN_CS_STOPPED)
    {
    ...
    {
        /* Return E_NOT_OK if the controller of the requested CanRxPduId is not in CAN_CS_STOPPED state */
        Status = E_NOT_OK;
    }

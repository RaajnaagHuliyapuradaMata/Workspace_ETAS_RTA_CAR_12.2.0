...

{
    ...
    if (config == NULL_PTR)
    {
        ...
        COMM_DET_REPORT_ERROR(COMM_INIT_APIID,COMM_E_WRONG_PARAMETERS);
        return;
    }
    /*********************************************DET END************************************************/
     ...
     * so as to ensure correct values for inhibition counters */
        ...
        if((Ret_NvM_Readval_u8 == E_OK  ) && (Ret_NvM_Error_Statusval_u8 == E_OK))
        {
            ...
            for (ChannelIndex_u8 = C_ZERO; ChannelIndex_u8 < COMM_NO_OF_CHANNELS; ++ChannelIndex_u8)
            {
                ...
                {
                    /*MR12 RULE 10.3 VIOLATION:  An expression of 'essentially Boolean' type (_Bool) is being converted
                     * to unsigned type, 'unsigned char' on assignment.
                    ...
                    channelRamPtr_pst->InhibitionReqStatus_u8 = ComM_Prv_GetNvMInhibitionStatus(ChannelIndex_u8,
                                                                ComM_NvMblock.ComMNoWakeup_u8);
                }
                else
                {

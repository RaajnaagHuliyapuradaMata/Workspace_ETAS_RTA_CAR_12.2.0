...
static void NvM_Prv_WriteAll_UpdateBlockData(void)
{
    ...
    // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2861] The maintenance operation is to be performed even if,
                                                     ...
                                                     NvM_Prv_Service_InitiationErrors_tuo *Errors_puo)
{
    ...
    boolean IsBlockWriteable_b = NvM_Prv_Block_IsWriteable(idBlock_uo,
                                                           NvM_Prv_Block_GetIdxDataset(idBlock_uo),
                                                           Errors_puo);
    if (!IsBlockWriteable_b)
    {
        ...
        // can be reported, clear the bit for write protected error before calling the ErrorDetection function.
        *Errors_puo &= ~(NvM_Prv_Service_InitiationErrors_tuo)NvM_Prv_Service_InitiationError_MaskBlkWriteProtected_e;
    }
    ...
    return (
            ...
            NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo, NVM_PRV_BLOCK_FLAG_SELECT_FOR_WRITE_ALL) &&
            ...
            (
             ...
             // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2978] Write block if WriteAll trigger is active
             IsTrgWriteAllActive_b
#if (NVM_RB_REQ_MONITOR == NVM_RB_REQ_MON_WRITE)
             // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3140] The NvM shall configure the NVRAM block NvM_WriteMon
...
static void NvM_Prv_WriteAll_FindBlocks(void)
{
    ...
    {
        // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3033] Ignore blocks with length null
        if (NvM_Prv_BlkDesc_GetSize(idBlock_uo) > 0u)
        {
            NvM_Prv_Service_InitiationErrors_tuo Errors_uo = 0u;

            if (NvM_Prv_WriteAll_IsBlockParticipating(idBlock_uo, &Errors_uo))
            {
            ...
            else
            {
                ...
                // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2879] If a block is currently PENDING
            ...
            // Calling foreign DET function is done out of the function NvM_Prv_WriteAll_IsBlockParticipating
            NvM_Prv_ErrorDetection_ReportServiceInitiationErrors(NVM_SERVICE_ID_WRITE_ALL,
                                                                 idBlock_uo,
                                                                 Errors_uo);

#if (NVM_PRV_MAINTAIN_REDUNDANT_BLOCKS)
            if (NVM_BLOCK_REDUNDANT == NvM_Prv_BlkDesc_GetType(idBlock_uo))
            {
                ...
                // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2860] A redundant block is maintained
                //                                               even if it is not suitable for WriteAll
                // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2869] A redundant block is to be maintained
                ...
                // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2862] The request result of a redundant block
...
static void NvM_Prv_WriteAll_Finalize(void)
{
    ...
    // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2804] If BswMMultiBlockJobStatusInformation is enabled,

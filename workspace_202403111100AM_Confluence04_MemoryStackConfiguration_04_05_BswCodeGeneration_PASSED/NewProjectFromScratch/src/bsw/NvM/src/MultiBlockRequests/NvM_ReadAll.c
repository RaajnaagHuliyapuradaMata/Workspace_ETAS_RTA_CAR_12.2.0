...
#if ((NVM_DYNAMIC_CONFIGURATION == STD_ON) && (NVM_RB_REMOVE_NON_RESISTANT_BLOCKS == STD_ON))
    //TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2999] the "NvM_Prv_isReadAllInitiated_b" will be set to TRUE
                                                    ...
                                                    NvM_Prv_Service_InitiationErrors_tuo* Errors_puo)
{
    ...
    return (
            ...
            NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo, NVM_PRV_BLOCK_FLAG_SELECT_FOR_READ_ALL)
           );
}
...
static void NvM_Prv_ReadAll_FindBlocks(void)
{
    ...
    for (idBlock_uo = NVM_CFG_FIRST_USED_BLOCK; idBlock_uo < NVM_CFG_NR_BLOCKS; ++idBlock_uo)
    {
        ...
        if (NvM_Prv_BlkDesc_GetSize(idBlock_uo) > 0u)
        {
            ...
            if (NvM_Prv_ReadAll_IsBlockParticipating(idBlock_uo, &Errors_uo))
            {
            ...
            else
            {
                ...
                // This is done right away to allow an immediate NvM_GetErrorStatus call returning a meaningful result
                // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2879] If a block is currently PENDING due
...
boolean NvM_Prv_IsSanitizedByReadAll(NvM_BlockIdType idBlock_uo)
{
    ...
    if (NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo, NVM_PRV_BLOCK_FLAG_SELECT_FOR_READ_ALL) &&
        !NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo, NVM_PRV_BLOCK_FLAG_RESISTANT_TO_CHANGED_SW))
    {
...
/* HIS METRIC STMT VIOLATION IN NvM_Prv_ReadAll_SetState: Due to the configuration this function may be empty */

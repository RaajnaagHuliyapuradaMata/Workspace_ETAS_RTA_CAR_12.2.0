 ...
 *typedef struct \n
 *{\n
 *  BufReq_ReturnType (*PduR_upTpCopyTxDataFunc) ( PduIdType gwId, const PduInfoType * info, const RetryInfoType* retry, PduLengthType* availableDataPtr);\n
 *}PduR_upTpCopyTxDataFuncType;\n
 */
typedef struct
{
/*   BufReq_ReturnType (*PduR_upTpProvideTxBufFunc) ( PduIdType, PduInfoType * , const RetryInfoType*,PduLengthType*);*/
    BufReq_ReturnType (*PduR_upTpCopyTxDataFunc) ( PduIdType gwId, const PduInfoType * info, const RetryInfoType* retry, PduLengthType* availableDataPtr);
}PduR_upTpCopyTxDataFuncType;
 ...
 *  PduLengthType                destPduLength;      Configured PduLength of the destPdu\n
 *  #if defined(PDUR_MODE_DEPENDENT_ROUTING) && (PDUR_MODE_DEPENDENT_ROUTING != 0) \n
 ...
 *  uint8                        gwType_u8;         Buffered gateway/ NonBuffer
 *  #endif \n
 *  } PduR_MT_UpToLo;
 */
typedef struct
{
   ...
   PduLengthType                destPduLength_uo;
   #if defined(PDUR_MODE_DEPENDENT_ROUTING) && (PDUR_MODE_DEPENDENT_ROUTING != 0)
...
/**
* @ingroup PDUR_TYPES_H
* Structure to store the metadata pointer + length of the metadata each source in a gwtp routing path
* typedef struct
*{
*    uint8* metaDataBuf;
*    uint8  metaDataBufLen;
*} PduR_MetaData_Info_tst;
**/
 ...
 * typedef struct\n
 *  {\n
 *  PduInfoType * pduPtr;   PDU info pointer  \n
 ...
 * typedef struct \n
 *  { \n
 *      PduLengthType                destPduLength;            Pdu length of Destinaation pdu\n
 *      PduIdType                    loId;              lower layer ID\n
 ...
 * typedef struct \n
 *  { \n
 *     const PduR_MetaData_Info_tst*  metaDataIFInfo_pst; points to a structure that has MetaDataPTR and MetaDataLen\n
 *     void * GW_If_buffer;    Points to either a PduInfoType (for a single entry buffer) or
...
typedef struct
{
   const PduR_MetaData_Info_tst*      metaDataIFInfo_pst;
   void * GW_If_buffer;
   PduIdType                    loId;
   PduR_loTransmitFP            transmitID;
   boolean                      isFifoType;
...
/**
* @ingroup PDUR_TYPES_H
* Structure to store total transferred length + read buffer pointer for each destination
* typedef struct
*{
*    uint8* readPtr_pu8;
*    PduLengthType  numTxLength_uo;
*} PduR_TpRP_Session_tst;
**/
 ...
 *  uint8 * metaDataPtr;    start address of meta data pointer \n
 ...
 * typedef struct \n
 *  { \n
 *      const PduR_FIFO_Tp_fixed_tst * GW_Tp_buffer;    Points to either a PduR_SingleEntryBuffer or a PduR_FIFO_fixed \n
 *      PduR_TpRP_Session_tst*    TpReadBuffInfo_pst;         Pointer to a structure which stores readInfoPtr and numTxLength for each destination
 *      PduR_MetaData_Info_tst*    MetaDataInfo_pst;         Pointer to a structure which stores metaDataPtr and metDataLength
 *      PduLengthType                numOfLowerLayers;     No.Of lower layers configured(To Check if its 1:n gateway multicast)
 *      FC_VariationPoint_START
 *      PduLengthType                TpGwQueueSize_uo;        Size of the Queue for a TP gateway if it is referring to a buffer pool\n
 *      FC_VariationPoint_END
 *      PduIdType                    loId;              lower layer ID \n
...
typedef struct
{
   const PduR_FIFO_Tp_fixed*   GW_Tp_buffer;
   PduR_TpRP_Session_tst*    TpReadBuffInfo_pst;
   const PduR_MetaData_Info_tst*      MetaDataInfo_pst;
 ...
 * typedef struct \n
 *  { \n
 *  uint32                    BytePosition;    Byte Position of the buffer \n
 ...
 * typedef struct \n
 *  { \n
 * PduRInfoType * gwBuf;       Points to PduR_Buf_If_Session \n
 * uint8 * begin;             starting address \n
 ...
 *  { \n
   ...
   const PduIdType*           LoTpTxToUpMapTable;    TxToUpMapTables are generated only for Cdd modules in order to differentiate IF and TX routing path\n
   const PduIdType*           LoTpRxToUpMapTable;    RxToUpMapTables are generated only for Cdd modules in order to differentiate IF and TX routing path\n
 ...
 *  { \n
   ...
   const PduIdType*           LoIfTxToUpMapTable; TxToUpMapTables are generated only for Cdd modules in order to differentiate IF and TX routing path\n
   const PduIdType*           LoIfRxToUpMapTable; RxToUpMapTables are generated only for Cdd modules in order to differentiate IF and TX routing path\n
 ...
 *      const PduR_RoutingPathGroupIdType          rpg_Idx;        Index to routing enable/disable status flag of RPG  \n
...
typedef struct
{
    ...
    const PduR_RoutingPathGroupIdType                         rpg_Idx;
 ...
 * typedef struct\n
 *  {\n
 *  const PduR_loTransmitFuncType * loTxTablePtr;           Pointer to PduR_loTransmitTable which points to various <Lo>_Transmit API. \n
 *  const PduR_loCancelTransmitFuncType * loCnclTxTablePtr;       Pointer to PduR_loCancelTransmitTable which points to various <Lo>_CancelTransmit API. \n
 *  const PduR_upIfRxIndicationFuncType * upIfRxIndTablePtr;      Pointer to PduR_upIfRxIndicationTable which points to various <UpIf>_RxIndication API. \n
 *  const PduR_upIfTxConfirmationFuncType * upIfTxConfTablePtr;     Pointer to PduR_upIfTxConfirmationTable which points to various <UpIf>_TxConfirmation API. \n
 *  const PduR_upTpCopyRxDataFuncType * upTpRxDataTablePtr;     Pointer to PduR_upTpCopyRxDataTable which points to various <UpTp>_CopyRxData API. \n
 *  const PduR_upTpStartOfReceptionFuncType * upTpStartRxTablePtr;    Pointer to PduR_upTpStartOfReceptionTable which points to various <UpTp>_StartOfReception API. \n
 *  const PduR_upTpRxIndicationFuncType * upTpRxIndTablePtr;      Pointer to PduR_upTpRxIndicationTable which points to various <UpTp>_RxIndication API. \n
 *  const PduR_upTpCopyTxDataFuncType * upTpTxDataTablePtr;     Pointer to PduR_upTpCopyTxDataTable which points to various <UpTp>_CopyTxData API. \n
 *  const PduR_upTpTxConfirmationFuncType * upTpTxConfTablePtr;     Pointer to PduR_upTpTxConfirmationTable which points to various <UpTp>_TxConfirmation API. \n
 *  const PduR_upIfTriggerTxFuncType * upIfTrigTxTablePtr;     Pointer to PduR_upIfTriggerTxTable which points to various <UpIf>_TriggerTransmit API. \n
 * } PduR_RPTablesType; \n
*/
typedef struct 
{
 ...
 *   const PduR_MT_LoIfTxToUp * loIfTxToUpMc;   lower If transmission to upper layer multicasting \n
 *   const PduR_MT_LoTpTxToUp * loTpTxToUpMc;   lower Tp transmission to upper layer multicasting \n
 *   PduR_MS_LoTpTxToUp * loTpTxToUpMs;   lower If transmission to upper layer \n
 ...
 *   PduR_RoutingPathGroupIdType*                           rpgId_MappingTab;
 ...
 * @ingroup PDUR_TYPES_H
 *
 * PduR internal Non buffer Type,Defined to uint8 value
 * This macro will be for multicast table [for ex. mcComToLo Multicast Transmission RP]
 */
#define PduR_NonBuffer 0u

/**
 * @ingroup PDUR_TYPES_H
 *
 * PduR internal Gw If buffer Type,Defined to uint8 value
 */
#define PduR_GwBuffer 1u

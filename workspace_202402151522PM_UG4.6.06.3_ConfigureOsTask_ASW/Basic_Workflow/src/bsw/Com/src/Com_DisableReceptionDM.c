...
void Com_Prv_DisableReceptionDM(Com_IpduGroupIdType idIpduGrp_u16)
{
    ...
    if(ipduGrpRam_pst->ipduGrpDMStatus_b != COM_STOP)
    {
        ...
        while (numOfIpdus_qu16 > COM_ZERO)
        {
            ...
            /* requested state is stop, decrement DM counter */
            ...
            * i.e., counter will have non-zero value */
            if (rxIpduRamPtr_pst->cntrStartedIpduGrpDM_u8 == COM_ZERO)
            {
                if (Com_GetRamValue(RXIPDU,_DMSTATUS,rxIpduRamPtr_pst->rxFlags_u16))
                {
                    Com_SetRamValue(RXIPDU,_DMSTATUS,rxIpduRamPtr_pst->rxFlags_u16, COM_STOP);
                }
            }
            ipduRefPtr_pcuo++;
            numOfIpdus_qu16--;

...
/**
 **************************************************************************************************
 * PduR_GF_Tp_TxConfirmation_Func - This function is called by a transport protocol module after the
 *                                  I-PDU has been transmitted on its network, the result will reveal
    ...
    if(gt->numOfLowerLayers == 1u)
    {
        ...
        if(ConnectionPtr->activeSessionCount == 1)
        {
            ...
            if((SessionPtr->Tx_Status == PDUR_TX_IDLE) &&
                    (SessionPtr->numRxLength_auo[SessionPtr->Tx_FifoInstanceUsed_u8] ==
                            SessionPtr->SduLength_auo[SessionPtr->Tx_FifoInstanceUsed_u8]))
            {
                ...
                /* As MetaDataPtr is not supported, In this task this will be initialized. So it will not hold any invalid address */
				readerPduInfo.MetaDataPtr = NULL_PTR;
                ...
                Ret_val =PDUR_BASE_RPTBL(loTxTablePtr)[(gt->loTransmitID)].
                        PduR_loTransmitFunc(gt->loId, &readerPduInfo);
                #else
        ...
        if(gt->TpQueueDepth_u8 > 1u)
        {
            ...
            while(BreakFlag_b)
            {
                ...
                if(SessionPtr->Tx_FifoInstanceUsed_u8 != SessionPtr->Rx_FifoInstanceUsed_u8)
                {
                    ...
                    SessionPtr->readPtr = fifo->begin;
                    TxInstanceLength_l =
                            (PduLengthType)((SessionPtr->Tx_FifoInstanceUsed_u8)*(SessionPtr->bufferLength));
                    ...
                    /* With return value E_OK, Set the Tx_Status to TX_INPROGRESS and move the readPtr
                     * to next instance of the buffer */
                    if(Ret_val == E_OK)
                    {
                        SessionPtr->Tx_Status = PDUR_TX_INPROGRESS;
                        BreakFlag_b = FALSE;
                    ...
                    else
                    {
                        ...
                        /* Returns E_NOT_OK, then I-PDU shall be removed from the FIFO and the next FIFO
                         * entry shall be transmitted, if available */
                        /* Move to next instance */
                        SessionPtr->Tx_FifoInstanceUsed_u8++;

                        /*[SWS_PDUR_00807, SWS_PduR_00806, SWS_PDUR_00816]:
                         ...
                         */
                        /* Since here PduR calls Det_ReportRuntimeError to report PDUR_E_PDU_INSTANCES_LOST,
                         * so the module id of the PduR module is passed as instance id during the call */
                        PDUR_REPORT_RUNTIME_ERROR(PDUR_MODULE_ID, PDUR_SID_GATEWAYTPTXCONFIRMATION,
                                PDUR_E_PDU_INSTANCES_LOST);
                    }
                }
                /* Rx_Status is required to check, otherwise for RxFifoUsed == TxFifoUsed, reset happens,
                 * but in between CopyRxData has received, then reset will lead to data lost */
                ...
                {
                    /* Set TxStatus to IDLE to trigger next transmission,
                     * when RxFifo = TxFifo and when RxStatus = IN_PROGRESS */
                    SessionPtr->Tx_Status = PDUR_TX_IDLE;

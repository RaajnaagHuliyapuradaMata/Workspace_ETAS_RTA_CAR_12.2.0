 ...
 **********************************************************************************************************************
 ...
 Description      : At invocation of Com_CopyTxData the AUTOSAR COM module copies the re-quested transmit data of the
                    ...
                    PduInfoPtr.
                    The function Com_CopyTxData also calculates and sets the availableDataPtr to the amount of
                    remaining bytes for the transmission of this large I-PDU. When TpDataState is TP_DATARETRY,
        ...
        * If PC variant is selected, then no mapping table will be used. */
        ...
        if (Com_GetRamValue(TXIPDU,_LARGEDATAINPROG,txIpduRamPtr_pst->txFlags_u16))
        {
            ...
            if (pduInfoPtr_pcst->SduLength != COM_ZERO)
            {
                ...
                if ((pduInfoPtr_pcst->SduLength + txIpduRamPtr_pst->txTpCopiedLength_uo) <=
                     txIpduRamPtr_pst->txTpLength_uo)
                {

                    /* Now Copy the Data from the IPDU buffer to SduDataPointer */
                    Com_ByteCopy((uint8*)(pduInfoPtr_pcst->SduDataPtr),
                                         (txIpduConstPtr_pcst->buffPtr_pu8 + txIpduRamPtr_pst->txTpCopiedLength_uo),
                    ...
                    * which is used as an offset for Ipdu buffer */
                    txIpduRamPtr_pst->txTpCopiedLength_uo += pduInfoPtr_pcst->SduLength;

                    /* Update the out parameter availableDataPtr, with the number of bytes remaining */
                    *availableDataPtr_puo = txIpduRamPtr_pst->txTpLength_uo - txIpduRamPtr_pst->txTpCopiedLength_uo;

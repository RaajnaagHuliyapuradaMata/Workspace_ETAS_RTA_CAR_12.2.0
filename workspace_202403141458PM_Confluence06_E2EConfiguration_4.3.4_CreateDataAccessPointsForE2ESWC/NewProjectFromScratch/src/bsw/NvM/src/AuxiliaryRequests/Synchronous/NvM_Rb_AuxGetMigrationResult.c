...
/**
 * TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3027]
 * Special handling for aux migration result
 *
 * Following request results are defined:
 * NVM_RB_MIGRATION_RESULT_INIT_E              (0u) =   Initialization value, result not yet available
 * NVM_RB_MIGRATION_RESULT_NOT_NECESSARY_E     (1u) =   RAM size == size on medium
 * NVM_RB_MIGRATION_RESULT_TO_SMALLER_SIZE_E   (2u) =   RAM size < size on medium (remaining data discarded)
 * NVM_RB_MIGRATION_RESULT_TO_BIGGER_SIZE_E    (3u) =   RAM size > size on medium (remaining data filled with zero)
 * NVM_RB_MIGRATION_RESULT_NOT_DONE_E          (4u) =   Migration not done, because data were not read
 * NVM_RB_MIGRATION_RESULT_DEACTIVATED_E       (5u) =   Migrations is deactivated by configuration
 */
...
{
    // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3053] Service to read the auxiliary migration result.
    ...
    if (NvM_Prv_ErrorDetection_IsNvmInitialized(NVM_SERVICE_ID_RB_AUX_GET_MIGRATION_RESULT, 0u))
    {
...
{
    // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3027] Calculate aux migration result
    ...
    {
        migrationResult_en = MemIf_Rb_GetMigrationResult(NvM_Prv_BlkDesc_GetIdDevice(idBlock_uo),

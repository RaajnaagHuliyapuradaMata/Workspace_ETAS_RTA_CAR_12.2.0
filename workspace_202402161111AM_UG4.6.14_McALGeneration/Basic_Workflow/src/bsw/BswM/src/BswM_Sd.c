    ...
    else
    {
        ...
        if(FALSE != flgDetChecksPassed_b)
        {
            ...
            if(FALSE != BswM_Prv_ReqProcessOrQueue_b(BSWM_MRP_SD_CNSMD_EVNT_GRP_CURR_STATE, SdConsumedEventGroupHandleId, idx_u16, (uint16)ConsumedEventGroupState))
            {
                ...
                BswM_Prv_RuleEval(dataSdConsEvntGrpCS_st.dataReqProcessing_en, BSWM_MRP_SD_CNSMD_EVNT_GRP_CURR_STATE, SdConsumedEventGroupHandleId, dataSdConsEvntGrpCS_st.adrRulesRef_pu16, dataSdConsEvntGrpCS_st.nrAssociatedRules_u16);

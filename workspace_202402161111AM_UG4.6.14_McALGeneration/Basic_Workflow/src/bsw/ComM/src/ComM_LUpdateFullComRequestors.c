 ...
 * Description      : Update FullComRequestors for the channels corresponding to the user requesting
...
void ComM_Prv_UpdateFullComRequestors(ComM_UserHandleType User_tu8, ComM_ModeType ComMode_tu8)
{
    ...
    {
        /* Update user in FullComRequestors if inhibition is active or not */
...
LOCAL_INLINE void ComM_Prv_DeleteUser(ComM_UserHandleType User_tu8)
{
    ...
    * copy back to fullcomrequestors */
    ...
    for(loopCount_u8 = C_ZERO;loopCount_u8 < numofChannels_u8;loopCount_u8++)
    {
        ...
        while ((searchIndex_u8 < COMM_MAX_CHANNEL_REQUESTERS) &&
                (User_tu8 != fullComRequestors_st.handleArray[searchIndex_u8]))
        {
        ...
        if(searchIndex_u8 < COMM_MAX_CHANNEL_REQUESTERS)
        {
            ...
            * Hence it is an incorrect warning reported by Misra */
            /*on sucessfull search, the searched user value is replaced with the next element in the array by shifting
            * the elements of the array*/
            for(nextElementIndex_u8=searchIndex_u8 + 1u; nextElementIndex_u8 < COMM_MAX_CHANNEL_REQUESTERS; searchIndex_u8++)
            {

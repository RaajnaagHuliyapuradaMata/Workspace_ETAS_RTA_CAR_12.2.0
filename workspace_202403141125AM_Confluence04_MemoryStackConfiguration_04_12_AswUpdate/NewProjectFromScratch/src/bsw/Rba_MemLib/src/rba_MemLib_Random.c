...
uint32 rba_MemLib_GetRandom_u32(uint16 nrRange_u16)
{
    ...
    Loc_d = Loc_a ^ ((Loc_a<< 5u) & 0xDA442D24u);
...
void rba_MemLib_SetRandomSeed(uint32 nrSeed_u32)
{
    ...
    for (idx=0u;idx<16u;idx++)
    {
        ...
        nrSeed_u32 = nrSeed_u32 * 9u;   // ignore overflow
    }
...
boolean rba_MemLib_IsRandomSeeded(void)
{
    ...
    flag = (boolean)(        (rba_MemLib_nrRandom[0u] !=0u));
    flag = (boolean)(flag || (rba_MemLib_nrRandom[3u] !=0u));

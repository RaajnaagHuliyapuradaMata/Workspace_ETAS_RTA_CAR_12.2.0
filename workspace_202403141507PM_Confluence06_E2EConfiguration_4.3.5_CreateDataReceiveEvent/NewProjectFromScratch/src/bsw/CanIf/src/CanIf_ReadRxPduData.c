        ...
        PduInfoType * CanIfRxInfoPtr)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NOT_OK((FALSE == CanIf_Prv_InitStatus_b), CANIF_READ_RX_PDU_DATA_SID, CANIF_E_UNINIT)
    ...
    if(((uint8)CAN_CS_STARTED == (((ControllerState_ps->Ctrl_Pdu_mode)&CANIF_BIT_MASK_PDU_MODE)>>CANIF_Ctrl_BIT_SHIFT)) && (((uint8)CANIF_ONLINE == ((ControllerState_ps->Ctrl_Pdu_mode)&CANIF_BIT_MASK_CTRL_MODE)) ||

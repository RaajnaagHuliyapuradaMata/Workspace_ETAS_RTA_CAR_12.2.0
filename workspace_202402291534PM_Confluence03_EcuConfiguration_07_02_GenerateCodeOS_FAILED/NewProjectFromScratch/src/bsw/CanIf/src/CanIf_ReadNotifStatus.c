                                                               ...
                                                               )
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NO_NOTIF((FALSE == CanIf_Prv_InitStatus_b), CANIF_RB_READ_TXPDU_CANID, CANIF_E_UNINIT)
...
CanIf_NotifStatusType CanIf_ReadRxNotifStatus(PduIdType CanIfRxSduId)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NO_NOTIF((FALSE == CanIf_Prv_InitStatus_b), CANIF_READ_RX_NOTIF_STATUS_SID,CANIF_E_UNINIT)
    ...
    lRxPduCustId_t = CanIf_Prv_ConfigSet_tpst->RxPduIdTable_Ptr[CanIfRxSduId];

    CANIF_DET_REPORT_ERROR_NO_NOTIF((lRxPduCustId_t == CANIF_INVALID_ID) ,CANIF_READ_RX_NOTIF_STATUS_SID, CANIF_E_INVALID_RXPDUID)


    /* Get the pointer to desired Rx pdu */
    RxPduCfg_pcst = CanIf_Prv_ConfigSet_tpst->RxPduConfig_pcst + lRxPduCustId_t;
                                        ...
                                        (RxPduCfg_pcst->RxPduReadNotifyReadDataStatus_u8 == CANIF_READ_NOTIFSTATUS))
    {
        ...
        NotifStatus_ten = *(RxNotifStatus_pen + lRxPduCustId_t);

        /* Reset the global variable holding the notification status */
        *(RxNotifStatus_pen + lRxPduCustId_t) = CANIF_NO_NOTIFICATION;

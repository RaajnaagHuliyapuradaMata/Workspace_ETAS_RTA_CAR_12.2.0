...
void    rba_MemLib_UpdateTimerUs(void)
{
    ...
    if (rba_MemLib_tiTimerUs_Incr == 0u)
    {
        rba_MemLib_tiTimerUs_Incr = 10u;
    }
...
void    rba_MemLib_UpdateTimerUs(void)
{
    ...
    else
    {
...
#if RBA_MEMLIB_USE_TIMER_CHECK != STD_OFF
        {
            ...
            ti_u32 = rba_MemLib_ConvertUsToTimerTicks(1u);
            if ( (ti_u32< 1u) || ( ti_u32 > 4000u) )
            {
                flgErr = TRUE;
            }
            ...
            if ((rba_MemLib_ConvertUsToTimerTicks(1000000u) & RBA_MEMLIB_TIMERBITMASK )< (ti_u32*1000000u))
            {
                flgErr = TRUE;
            }

            /* Check whether timer is running AND no error -> leave INIT */
            if ((!flgErr) && (tiTicksOld > 0u))
            {

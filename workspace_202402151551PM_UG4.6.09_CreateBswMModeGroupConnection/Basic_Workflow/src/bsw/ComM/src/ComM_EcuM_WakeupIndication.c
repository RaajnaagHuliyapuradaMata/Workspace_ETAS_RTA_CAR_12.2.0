...
void ComM_EcuM_WakeUpIndication(NetworkHandleType Channel)
{
    ...
    if (ComM_GlobalStruct.ComM_InitStatus_en != COMM_INIT)
    {
        ...
        COMM_DET_REPORT_ERROR(COMM_ECUM_WAKEUP_INDICATION, COMM_E_NOT_INITED);
        return;
    }
    ...
    if (Channel >= COMM_NO_OF_CHANNELS)
    {
        ...
        COMM_DET_REPORT_ERROR(COMM_ECUM_WAKEUP_INDICATION, COMM_E_WRONG_PARAMETERS);
        return;
    }
    /*********************************************DET END************************************************/
    ...
    if(channelRamPtr_pst->ChannelMode_u8 == COMM_NO_COMMUNICATION)
    {
        ...
        if(channelConfigPtr_pcst->ComMMasterChannelPresent_b == TRUE)
        {
            ...
            * This expression is not true always. This expression value gets updated in the channel state machine for
            * every main function.

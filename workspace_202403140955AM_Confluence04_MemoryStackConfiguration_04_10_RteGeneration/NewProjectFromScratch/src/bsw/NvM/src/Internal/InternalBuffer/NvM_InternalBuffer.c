...
// TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3029]
// TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3030]
                                                      ...
                                                      boolean CopyIntoBuf_b)
{
    ...
    rba_MemLib_MemCopy(src_pcu8, dst_pu8, NvM_Prv_BlkDesc_GetSize(idBlock_uo));
...
# if (defined(NVM_PRV_HAS_RAM_MIRROR) && (NVM_PRV_HAS_RAM_MIRROR == STD_ON))
    ...
    uint32 i;
    for (i = 0u; i < sizeof(NvM_Prv_RamMirror_au8); ++i)
    {
        NvM_Prv_RamMirror_au8[i] = 0u;
    }

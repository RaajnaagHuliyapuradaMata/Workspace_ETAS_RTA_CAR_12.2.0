...
boolean EcuM_Prv_CheckWakeupTimerHandling ( void )
     ...
     EcuM_ShutdownModeType dataSleepModeHandle_u16 = ECUM_SLEEP_MAX_U16;
     ...
     for( ctrLoop1_u8 = 0;ctrLoop1_u8 < ECUM_NUM_OF_WKP_SOURCES;ctrLoop1_u8++ )
     {
         ...
         /*Check if any timer needs to be decremented for any wakeup*/
                 ...
                 && (dataCheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupTimeout > ECUM_TIMEOUT_U16))
         {
             SchM_Enter_EcuM();           /*Enter Critical Section*/
             /*Decrement the timer */
             EcuM_Cfg_CheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupTimeout=EcuM_Cfg_CheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupTimeout - ECUM_TIMER_REDUCTION_U16;
             /*If timeout occurs, update the status for the corresponding wakeup to timer expired*/
             if(EcuM_Cfg_CheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupTimeout == ECUM_TIMEOUT_U16)
             {
     ...
     if (flgExitSleep_u8 == ENTER_SLEEP_E)
         {
             ...
             *  wakeups that had been detected has expired ,
     ...
     else if(flgExitSleep_u8 == EXIT_SLEEP_E)
     {
         ...
         EcuM_Cfg_CheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupTimeout = ECUM_TIMEOUT_U16;
         }
         SchM_Exit_EcuM();        /*Leave Critical Section*/
     }
     else
...
/***********************************************************************************
  ...
  Description   :   DET error is reported when CheckWakeupTimer expired for
                      all asynchronous wakeups and hence check was unsuccessful.
  Parameter in  :   None
  Parameter out :   None
  Return value  :   None
  Remarks       :
...
static void EcuM_Prv_ReportCheckWakeupError ( void )
    ...
    SchM_Exit_EcuM();        /*Leave Critical Section*/
        ...
        for( ctrLoop1_u8 = 0;ctrLoop1_u8 < ECUM_NUM_OF_WKP_SOURCES;ctrLoop1_u8++ )
        {
             if(dataCheckWakeupTimer_ast[ctrLoop1_u8].CheckWakeupStatus == ECUM_TIMER_EXPIRED_E)
            {
...
#if (ECUM_DEV_ERROR_DETECT == STD_ON)
           if( (cntrExp_u8 != 0x00) && (ECUM_NUM_OF_WKP_SOURCES==(cntrExp_u8 + cntrReset_u8)) )
            {
               /*report error if all wake up sources CheckWakeupTimer expired and hence check was unsuccessful*/
            ...
            else
            {
                 ...
                 * CheckWakeupTimer for all of the async wakeups that had been detected has expired.
                 * In this case EcuM_Prv_SleepReqType_en will never be ECUM_PRV_NO_SLEEP_E*/
            }
...
static void EcuM_Prv_ProceedSleep( uint16 dataSleepModeHandle_u16 )
...
#if(ECUM_CFG_MULTICORE_ENABLED  ==  STD_ON)
    uint16 cntrLoopOsCore_u16 = 0x00;
    /*Get the active core id from Os*/
    ...
    EcuM_Prv_CoreStatus_st.CurrentCoreID= ECUM_COREID_DEFAULT_U16;//initialise with invalid value
#endif//ECUM_CFG_MULTICORE_ENABLED
#if(ECUM_CFG_MULTICORE_ENABLED == STD_ON)
...
static void EcuM_Prv_ProceedShutdown(void)
{
      ...
      uint32 EcuM_dataValidatedWakeupEvents_u32 = ECUM_NO_WKUP_SRC;
      // internal variable used to hold the wakeup events with validation timeout
      uint32 EcuM_dataPendingWakeupEvents_u32 = ECUM_NO_WKUP_SRC;
#if (ECUM_CFG_NVM_BLOCK_CONFIGURED == TRUE)
       // Variable to hold the NvM request result
      NvM_RequestResultType EcuM_NvM_RequestResult_u8 = NVM_REQ_OK ;
      // Variable to hold the status of NvM Write operation
      Std_ReturnType retNvMErrorStatus_u8 = E_NOT_OK;
#endif
      // Flag variable hold the info of change in shutdown target in case of wakeup
      boolean EcuM_flgShutdowntargetchange_b = FALSE;
...
#if(ECUM_IGNORE_WAKEUPEVVAL_OFFPREOS == STD_ON)

            /* when EcuMIgnoreWakeupEvValOffPreOS is True only without Validation is Consider, rest of the wakeup event are ignore*/
            EcuM_dataValidatedWakeupEvents_u32 = EcuM_GetValidatedWakeupEvents();
            if  ( ECUM_NO_WKUP_SRC != EcuM_dataValidatedWakeupEvents_u32)
            {
                /*Shutdown target is set to RESET */
                 EcuM_Prv_dataSelectedShutdownTarget_st.ShutdownTarget = ECUM_SHUTDOWN_TARGET_RESET;

                 EcuM_Prv_dataSelectedShutdownTarget_st.mode = EcuMConf_EcuMResetMode_ECUM_RESET_MCU;

                 EcuM_flgShutdowntargetchange_b = TRUE;
            }

#else  // ECUM_IGNORE_WAKEUPEVVAL_OFFPREOS Else

            /* when EcuMIgnoreWakeupEvValOffPreOS is false both with and without validation is consider */
            EcuM_dataValidatedWakeupEvents_u32 = EcuM_GetValidatedWakeupEvents();

            EcuM_dataPendingWakeupEvents_u32 = EcuM_GetPendingWakeupEvents();

            if ( (ECUM_NO_WKUP_SRC !=EcuM_dataValidatedWakeupEvents_u32  ) || ( ECUM_NO_WKUP_SRC != EcuM_dataPendingWakeupEvents_u32) )
            {
                                 /*Shutdown target is set to RESET */
                EcuM_Prv_dataSelectedShutdownTarget_st.ShutdownTarget = ECUM_SHUTDOWN_TARGET_RESET;
...
#endif // ECUM_IGNORE_WAKEUPEVVAL_OFFPREOS
#if (ECUM_CFG_NVM_BLOCK_CONFIGURED == TRUE)
            if(TRUE == EcuM_flgShutdowntargetchange_b)
            {
                ...
                /*Write the RAM block status for shutdown info block of NvM , In BlockDescriptor, permanent RamBlockDataAddress is forwarded, So only in NvM_WriteBlock parameter pointer as NULL_PTR   */
                retNvMErrorStatus_u8 = NvM_WriteBlock((NvM_BlockIdType) NvMConf_NvMBlockDescriptor_ECUM_CFG_NVM_BLOCK, NULL_PTR );
                if (E_OK == retNvMErrorStatus_u8 )
                {
                    do
                    {
                        /*Schedule main functions necessary for NvM operation*/
                        NvM_MainFunction();
                        MemIf_Rb_MainFunction();

                        /* Get the error/status of write information for shutdown info block of NvM*/
                        retNvMErrorStatus_u8 = NvM_GetErrorStatus( (NvM_BlockIdType) NvMConf_NvMBlockDescriptor_ECUM_CFG_NVM_BLOCK, & EcuM_NvM_RequestResult_u8 );

                        if (E_NOT_OK == retNvMErrorStatus_u8)
                        {
                            /* Block Id or Pointer are Invalid */
                            break;
                        }
                        else
                        {
                    ...
                    }while(NVM_REQ_PENDING == EcuM_NvM_RequestResult_u8);

                    if ( NVM_REQ_OK != EcuM_NvM_RequestResult_u8)
                    {
                        EcuM_ErrorHook( ECUM_E_NVM_WRITEBLOCK_ERROR ); // Error hook is raised due to failed to write the shutdown info to NVRAM.
                    }
                    else
                    {
                        /* RAM block sucessfully update to NVRAM */
                     }
                }
                else
                {
                    EcuM_ErrorHook( ECUM_E_NVM_WRITEBLOCK_REQUEST_FAILED ); // Error hook is raised during initiation of the write request from the RAM block has failed.
                }
            }
#endif//ECUM_CFG_NVM_BLOCK_CONFIGURED
...
static void EcuM_Prv_WakeupValidation(void)
    ...
    if ((datawkpSrcValidationTimeoutCtr_u16 == ECUM_TIMEOUT_U16) &&
...
static void EcuM_Prv_RunHandling(void)
{
    ...
    if( EcuM_Prv_RunMinDuration_u16 > ECUM_TIMEOUT_U16 )
    {
        EcuM_Prv_RunMinDuration_u16 = EcuM_Prv_RunMinDuration_u16 - ECUM_TIMER_REDUCTION_U16;
    }
     else
    {
    ...
    /*Loop for handling the RUN requests from the Users*/
        for( cntrLoopCtr_u8 = 0; cntrLoopCtr_u8 <ECUM_CFG_NUM_FLEX_USERS;cntrLoopCtr_u8++ )
        {
            ...
            if( EcuM_Prv_userRUNReqStatus_ab[ cntrLoopCtr_u8 ] == TRUE )
            {
                ...
                {
                    BswM_EcuM_RequestedState( ECUM_STATE_RUN,ECUM_RUNSTATUS_REQUESTED );
        ...
        if( ( EcuM_Prv_RunMinDuration_u16 == ECUM_TIMEOUT_U16 ) && ( UserRUNReqPending_b == FALSE )
                && ( ( EcuM_Prv_RunRequested_u8 == ECUM_RUNSTATUS_REQUESTED ) ||  ( EcuM_Prv_RunRequested_u8 == ECUM_RUNSTATUS_UNKNOWN ) ) )
        {
            BswM_EcuM_RequestedState( ECUM_STATE_RUN,ECUM_RUNSTATUS_RELEASED );
    ...
    /*Loop for handling the POSTRUN requests from the Users*/
        for( cntrLoopCtr_u8 = 0; cntrLoopCtr_u8 < ECUM_CFG_NUM_FLEX_USERS;cntrLoopCtr_u8++ )
        {
            ...
            if( EcuM_Prv_userPostRUNReqStatus_ab[ cntrLoopCtr_u8 ] == TRUE )
            {
                ...
                {
                    BswM_EcuM_RequestedState( ECUM_STATE_POST_RUN,ECUM_RUNSTATUS_REQUESTED  );
        ...
        /*BswM Notification Status Released to be triggered, when all the user requests are released. Run minimum duration check
        is to ensure the idle time as well as warm-up time to prepare the RTE and application software users.*/
        if( ( EcuM_Prv_RunMinDuration_u16 == ECUM_TIMEOUT_U16 ) && ( UserPostRUNReqPending_b == FALSE )
                && ( ( EcuM_Prv_Post_Run_Requested_u8 == ECUM_RUNSTATUS_REQUESTED ) || ( EcuM_Prv_Post_Run_Requested_u8 == ECUM_RUNSTATUS_UNKNOWN ) ) )
        {
            BswM_EcuM_RequestedState( ECUM_STATE_POST_RUN,ECUM_RUNSTATUS_RELEASED );

    ...
    for (hasIndex_u16 = 0; hasIndex_u16 < dataActionList_pst->nrActionListItems_u8; hasIndex_u16++)
    {
          ...
          *  member, was already executed before. If yes, then dont call BswM_Prv_Evaluate_Action and dont execute the action*/
...
#endif
             {
			  ...
			  if ((dataActionListItem_pst->actionListItemErrorId_u8 != 0) && (isActionRet_u8 == E_NOT_OK))
			  {
				#if (BSWM_DEV_ERROR_DETECT == STD_ON)
					#define BSWM_E_ACTION_FAILED dataActionListItem_pst->actionListItemErrorId_u8
                   /* DET if the BSW Mode Manager is not initialized.*/
                   BSWM_DET_REPORT_ERROR((uint8)dataActionListItem_pst->dataActionListItemType_en, BSWM_E_ACTION_FAILED);
				#endif
			  }
              if (dataActionList_pst->nrActionListItems_u8 == (hasIndex_u16 + 1))
              {

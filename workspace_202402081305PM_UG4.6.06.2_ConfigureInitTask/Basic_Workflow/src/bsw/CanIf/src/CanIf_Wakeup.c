...
Std_ReturnType CanIf_CheckWakeup (EcuM_WakeupSourceType WakeupSource)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NOT_OK((FALSE == CanIf_Prv_InitStatus_b), CANIF_CHECKWAKEUP_SID, CANIF_E_UNINIT)
    ...
    if(((((lControllerState_p->Ctrl_Pdu_mode)&CANIF_BIT_MASK_PDU_MODE)>>CANIF_Ctrl_BIT_SHIFT) == (uint8)CAN_CS_SLEEP) && (WkpDeviceFound_b == TRUE))
    {
        /*[SWS_CANIF_00720] Invoke Wakeup functions */
        #if THIRD_PARTY_CANTRCV_DRIVER_INTEGRATION == STD_ON
        Ret_trcv_t = CanTrcv_Integn_CheckWakeup((WkpEvent_tpcst +LoopVar_tqu8)->TransceiverId);
        #else
...
Std_ReturnType CanIf_CheckValidation (EcuM_WakeupSourceType WakeupSource)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NOT_OK((FALSE == CanIf_Prv_InitStatus_b), CANIF_CHECKVALIDATION_SID, CANIF_E_UNINIT)
    ...
    if(((((ControllerState_ps->Ctrl_Pdu_mode)&CANIF_BIT_MASK_PDU_MODE)>>CANIF_Ctrl_BIT_SHIFT) == (uint8)CAN_CS_STARTED) && (WkpDeviceFound_b ==E_OK))
    {

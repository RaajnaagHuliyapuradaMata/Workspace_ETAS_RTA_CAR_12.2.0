...
void Com_Prv_TxChangeMode(Com_IpduId_tuo idTxIpdu_uo)
{
    ...
    if (COM_TX_MODE_IS_CYCLIC(latestTransMode_u8))
    {
        ...
        * Com_MainFunctionTx(), So Interrupt is locked here. */
        txIpduRamPtr_pst->cntrTimePeriod_u16 = COM_GET_TXIPDU_TIMEPERIOD(idTxIpdu_uo,
...
LOCAL_INLINE void Com_Prv_TxChangeMode_Signal(Com_IpduId_tuo idTxIpdu_uo)
{
    ...
    for (index_qu16 = txIpduConstPtr_pcst->numOfSig_u16; index_qu16 != COM_ZERO; index_qu16--)
    {
        ...
        if (filterAlg_u8 != COM_NOTCONFIGURED)
        {
            constByteValue_u8 = txSigConstPtr_pcst->general_u8;

            /* unpack signal data only for filter algorithms where value is required for processing the filter,
            i.e., except for ALWAYS, NEVER, ONE_EVERY_N */
            if ((filterAlg_u8 > COM_NEVER) && (filterAlg_u8 != COM_ONE_EVERY_N))
            {
                sigType_u8 = Com_GetValue(GEN,_TYPE,constByteValue_u8);

                /* Locks are required here,to protect Tx Ipdu buffer due to the signal updates as per below
                * requirement.
            ...
            filterVal_b = Com_Prv_Filter((uint32)txNewVal_uo,filterAlg_u8, txSigConstPtr_pcst->idxFilter_uo,
                                                                                         COM_PRV_FROM_IPDU_GRP_START);
            ...
            Com_SetRamValue(TXSIG,_SIGCHNGMODE,txSigConstPtr_pcst->sigRAM_pst->txSigRAMFields_u8,filterVal_b);
        }
        txSigConstPtr_pcst++;
    }
}
    ...
    for (idxSigGrp_qu16 = txIpduConstPtr_pcst->numOfSigGroups_u16; idxSigGrp_qu16 > COM_ZERO; idxSigGrp_qu16--)
    {
        if (Com_GetValue(TXSIGGRP,_FILTEREVALREQ,txSigGrpConstPtr_pcst->txSignalGrpFields_u8))
        {
            ...
            for (idxGrpSig_qu16 = txSigGrpConstPtr_pcst->numOfGrpSig_uo; idxGrpSig_qu16 > COM_ZERO; idxGrpSig_qu16--)
            {
                ...
                if (txGrpSigConstPtr_pcst->filterAlgo_u8 != COM_NOTCONFIGURED)
                {
                    filterVal_b = Com_Prv_TxChangeMode_GroupSignal(idTxIpdu_uo, txGrpSigConstPtr_pcst);

                    /* Store the latest filter state of the signal */
                    Com_SetRamValue(TXGRPSIG,_SIGCHNGMODE,
                                             txGrpSigConstPtr_pcst->grpSigRAM_pst->txGrpSigRAMFields_u8,filterVal_b);
                }
...
# endif
    ...
    filterAlgo_u8 = txGrpSigConstPtr_pcst->filterAlgo_u8;

    /* unpack signal data only for filter algorithms where value is required for processing the filter,
    i.e., except for ALWAYS, NEVER, ONE_EVERY_N */
    if ((filterAlgo_u8 > COM_NEVER) && (filterAlgo_u8 != COM_ONE_EVERY_N))
    {
# ifdef COM_TX_SIGNALGROUP_ARRAY
...
# endif /* #ifdef COM_TXGRPSIG_INT64 */
    ...
    filterVal_b = Com_Prv_Filter((uint32)txNewVal_uo, filterAlgo_u8, txGrpSigConstPtr_pcst->idxFilter_uo,
                                                                                        COM_PRV_FROM_IPDU_GRP_START);
...
boolean Com_Prv_SigTxChangeMode(Com_SignalIdType idSignal_u16, uint32 txNewVal_u32)
{
    ...
    txIpduRamPtr_pst        = COM_GET_TX_IPDU_CONSTDATA(idTxIpdu_uo)->ipduRam_pst;
    filterAlg_u8            = (uint8)Com_GetValue(TXSIG,_FILTRALG,txSigConstPtr_pcst->txSignalFields_u16);
    SchM_Enter_Com_TxIpduProtArea(SIGTXCHANGEMODE);

    /* Check the configured filter algorithm */
    if ( filterAlg_u8 > COM_NEVER)
    {
        /* Evaluate current filter state of the signal */
        sigTMSState_b = Com_Prv_Filter(txNewVal_u32,filterAlg_u8,txSigConstPtr_pcst->idxFilter_uo,
                                       COM_PRV_NOT_FROM_IPDU_GRP_START);

        /* Update TMS TRUE counter, only if filter state of the signal changes.
        * Otherwise, TMS TRUE counter value remains unchanged. */
        if (Com_GetRamValue(TXSIG,_SIGCHNGMODE,txSigConstPtr_pcst->sigRAM_pst->txSigRAMFields_u8) != sigTMSState_b)
        {
            if (sigTMSState_b)
            {
            ...
            Com_SetRamValue(TXSIG,_SIGCHNGMODE,txSigConstPtr_pcst->sigRAM_pst->txSigRAMFields_u8,sigTMSState_b);
        }
    }
...
boolean Com_Prv_SigGrpTxChangeMode(Com_SignalGroupIdType idSigGroup_u16)
{
    ...
    for (idxGrpSig_qu16 = txSigGrpConstPtr_pcst->numOfGrpSig_uo; idxGrpSig_qu16 > COM_ZERO; idxGrpSig_qu16--)
    {
        ...
        * then the Value will be COM_ALWAYS in the configuration */
        if ((txGrpSigConstPtr_pcst->filterAlgo_u8 != COM_NOTCONFIGURED) &&
            (txGrpSigConstPtr_pcst->filterAlgo_u8 > COM_NEVER))
        {
            constByteValue_u16  = txGrpSigConstPtr_pcst->txGrpSigFields_u8;
            sigType_u16         = (Com_GetValue(TXGRPSIG,_TYPE,constByteValue_u16));
            ...
            sigTMSState_b = Com_Prv_Filter((uint32)txNewValGrpSig_uo, txGrpSigConstPtr_pcst->filterAlgo_u8,
                                                               txGrpSigConstPtr_pcst->idxFilter_uo,
                                                               COM_PRV_NOT_FROM_IPDU_GRP_START);

            /* Update TMS TRUE counter , only if filter state of the signal changes.
            * Otherwise, TMS TRUE counter value remains unchanged.
            */
            if (Com_GetRamValue(TXGRPSIG,_SIGCHNGMODE,txGrpSigConstPtr_pcst->grpSigRAM_pst->txGrpSigRAMFields_u8) !=
                                                                                                        sigTMSState_b)
            {
                if (sigTMSState_b)
                {
                ...
                Com_SetRamValue(TXGRPSIG,_SIGCHNGMODE,txGrpSigConstPtr_pcst->grpSigRAM_pst->txGrpSigRAMFields_u8,
                                                                                                        sigTMSState_b);
            }
        }
        txGrpSigConstPtr_pcst++;

...
void PduR_BufferPoolInit(const PduR_BufferPool_ConfigType * BuffPoolConfigPtr)
{
    ...
    PduRBufferPoolLengthType PoolBuffer_Idx;    /* Local variable which holds each buffer's index
...
BufReq_ReturnType PduR_BufferPoolRequest(uint16 BuffPoolId, PduLengthType TpSduLength,
        PduLengthType * PduRBufferSizePtr, uint8 * *bufferPtr)
{
    /* Start: Variable declaration */
    PduR_BufferPool_FixedPtr BufferPoolConstPtr;
    PduR_BufferPool_DynamicPtr BufferPoolDynPtr;
            ...
            for(PoolBuffer_Idx = 0; PoolBuffer_Idx < BufferPoolConstPtr->totalBuffersInPool; PoolBuffer_Idx++)
            {
                ...
                )
                {
                    ...
                    *PduRBufferSizePtr = BufferPoolConstPtr->poolBufferConst[PoolBuffer_Idx].poolBufferLength;
                    /* Copy the allocated buffer's pointer into bufferPtr out parameter, this is the actual
                     * buffer to be used for gatewaying */
                    *bufferPtr = &(BufferPoolConstPtr->poolBufferConst[PoolBuffer_Idx].poolBufferPtr[1]);
                    ...
                    break;
                }/* end: (TpSduLength <= FixedPtr->poolBufferConst[index].poolBufferLength#ifndef
                  * PDUR_STORE_AND_FORWARD && FixedPtr->poolBufferDyn[index].poolBufferStatus == TRUE #endif )*/
                else
                {
...
void PduR_BufferPoolRelease(uint16 BuffPoolId, const uint8 * bufferPtr)
{
    ...
    PduR_BufferPool_DynamicPtr BufferPoolDynPtr;
    PduRBufferPoolLengthType    FreeBuffersInPool;
    PduRBufferPoolLengthType PoolBuffer_Idx;    /* Local variable which holds each buffer's
     * index inside the buffer pool */
    /* End: Variable declaration */

...
void CANIF_TXCONFIRMATION_FNAME(PduIdType CanTxPduId)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_VOID_RET((FALSE == CanIf_Prv_InitStatus_b), CANIF_TX_CONFIRMATION_SID, CANIF_E_UNINIT)
...
if( ((lControllerState_p->CanIf_TxCnfmStatus) != CANIF_TX_RX_NOTIFICATION) && ((((lControllerState_p->Ctrl_Pdu_mode)&CANIF_BIT_MASK_PDU_MODE)>>CANIF_Ctrl_BIT_SHIFT) == (uint8)CAN_CS_STARTED))
{
    ...
    if (CanIf_Prv_TxBufferRam_ast[lBufferId].bufferstatus  != CANIF_TXBUFFER_EMPTY)            /*Buffer is NOT empty*/
    {
        ...
        /*[SWS_CANIF_00183]*/
               ...
               *(lDataBuf_p + ((lCanIdBuf_p[lClearIndex].BufferIndex)*(lDataMaxLen+1u)) + 0u)    = 0xFFu;       /*Index is available*/


                 CanIf_Prv_TxPduRam_ast[(lCanIdBuf_p + lClearIndex)->swPduHandle].pdu_buffered_flag = FALSE;
                 (lCanIdBuf_p + lClearIndex)->BufferIndex = 0u;
                  (lCanIdBuf_p + lClearIndex)->CanId = 0u;
                  (lCanIdBuf_p + lClearIndex)->SduLength = 0u;
                 (lCanIdBuf_p + lClearIndex)->swPduHandle = 0u;

                 (CanIf_Prv_TxBufferRam_ast[lBufferId].last_index) = (lClearIndex - 1u)&0xFFu;
...
Std_ReturnType CanIf_XCore_LocalCore_TxConfirmation(const CanIf_Cfg_TxPduConfig_tst * CanIf_TxPduConfig_pst)
{
    ...
    /* Provide confirmation callback to upper layers */

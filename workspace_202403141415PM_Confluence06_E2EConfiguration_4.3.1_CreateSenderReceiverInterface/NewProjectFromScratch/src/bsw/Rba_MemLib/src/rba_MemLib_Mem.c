...
void rba_MemLib_MemCopy_Provided(uint8 const * src_pcu8, uint8 * dst_pu8, uint32 length_u32)
{
    ...
    if (lenRemaining_u32 >= 4u)
    {
        ...
        /* MR12 RULE 11.4 VIOLATION: Casts to uint32 needed to perform following alignment checks on the addresses - not critical */
        bytesUnalignedPreSrc_u32  = (uint32)curSrc_pcu8;
        bytesUnalignedPreSrc_u32 &= 3u;
        bytesUnalignedPreDst_u32  = (uint32)curDst_pu8;
        bytesUnalignedPreDst_u32 &= 3u;
        ...
        {
            rba_MemLib_UintPtr_tuo  addr_uo;
            ...
            bytesUnalignedPre_u32 = (4u - bytesUnalignedPreSrc_u32) & 0x3u;
            ...
            bytesAligned_u32 = (uint32)(~(uint32)3u & lenRemaining_u32);

            /* explicitely convert the pointers to uint32 as the alignment is 32bit aligned */
            /* MR12 RULE 11.4 VIOLATION: Cast of Pointers via integer is necessary as following access is performed with 32bit alignments. Alignment was ensured before. Conversion uses special integer type rba_MemLib_UintPtr_tuo */
            addr_uo      = (rba_MemLib_UintPtr_tuo)curSrc_pcu8;
            curSrc_pcu32 = (uint32 const *)addr_uo;
            addr_uo      = (rba_MemLib_UintPtr_tuo)curDst_pu8;
            curDst_pu32  = (uint32       *)addr_uo;
...
/** ***********************************************************************
 * \brief               Copy data from src_pcu8 to dst_pu8 in uint8 steps (decreasing addresses)
 *
 * \param   src_pcu8    Pointer to the source buffer start - no alignment constraint
 * \param   dst_pu8     Pointer to the destination buffer start - no alignment constraint
 * \param   length_u32  Number of bytes to copy (0 allowed)
 */
void rba_MemLib_MemCopy_Backwards(uint8 const * src_pcu8, uint8 * dst_pu8, uint32 length_u32)
{
    uint8   const   *   curSrc_pcu8;
    uint8           *   curDst_pu8;
    uint32              lenRemaining_u32;
    ...
    /* Prepare running variables - note: parameters shall not be modified */
    curSrc_pcu8         = src_pcu8 + length_u32;
    curDst_pu8          = dst_pu8  + length_u32;
    lenRemaining_u32    = length_u32;
    ...
    if (lenRemaining_u32 >= 4u)
    {
        ...
        if (bytesUnalignedPreSrc_u32 == bytesUnalignedPreDst_u32)
        {
            ...
            /* Perform the Pre-Copy (decrementing) */
            rba_MemLib_MemCopyU8U8_Dec(curSrc_pcu8, curDst_pu8, bytesUnalignedPre_u32);

            /* Determine the new copy-base (pointer decrementing)) */
            curSrc_pcu8         -= bytesUnalignedPre_u32;
            curDst_pu8          -= bytesUnalignedPre_u32;
            lenRemaining_u32    -= bytesUnalignedPre_u32;
            ...
            /* explicitely convert the pointers to uint32 as the alignment is 32bit aligned */
            /* MR12 RULE 11.4 VIOLATION: Pointer to/from integer is necessary as following access is performed with 32bit alignments. That was ensured before -> not critical */
            addr_uo      = (rba_MemLib_UintPtr_tuo)curSrc_pcu8;
            curSrc_pcu32 = (uint32 const *)addr_uo;
            addr_uo      = (rba_MemLib_UintPtr_tuo)curDst_pu8;
            curDst_pu32  = (uint32       *)addr_uo;

            /* Copy data in 32bit granularity (decrementing) */
            rba_MemLib_MemCopyU32U32_Dec(
                    curSrc_pcu32,
                    curDst_pu32,
                    bytesAligned_u32
            );
    ...
    /* Unaligned bytes post copy (decreasing order) */
    rba_MemLib_MemCopyU8U8_Dec(curSrc_pcu8, curDst_pu8, lenRemaining_u32);
    return;
}
 ...
 * \param   length_u32  Number of bytes to copy (0 allowed)
*/
void rba_MemLib_MemCopyOverlap(uint8 const *src_pcu8, uint8 *dst_pu8, uint32 length_u32)
{
    uint8 const *srcEnd1_pcu8;

    /* Address END+1 = address AFTER the copied buffer */
    srcEnd1_pcu8 = src_pcu8 + length_u32;

    /* Note: Checking for dest start >  source END is the same as
     *                    dest start >= source END+1
     * but makes the SW algo simpler
     */

    /* Forward-copy strategy (Standard) shall be selected if
           destination start address <= source start address  (still overlap possible but no issue with inc-pointer strategy)
        OR destination start address >= source END+1 address   (no overlap possible)
    */
    if ( (dst_pu8 <= src_pcu8) || (dst_pu8 >= srcEnd1_pcu8) )
    {
        /* Increasing address strategy selected */
        rba_MemLib_MemCopy_Provided(src_pcu8, dst_pu8, length_u32);
    }
    else
    {
        /* Decreasing strategy selected */
        rba_MemLib_MemCopy_Backwards(src_pcu8, dst_pu8, length_u32);
    }
    return;
}
    ...
    if (lenRemaining_u32  >= 4u)
    {
        ...
        if (bytesUnalignedPreSrc_u32 == bytesUnalignedPreDst_u32)
        {
            ...
            if (flgRet)
            {
                ...
                /* MR12 RULE 11.4 VIOLATION: Cast of Pointers via integer is necessary as following access is performed with 32bit alignments. Alignment was ensured before. Conversion uses special integer type rba_MemLib_UintPtr_tuo */
                addr_uo      = (rba_MemLib_UintPtr_tuo)curBfr1_pcu8;
                curBfr1_pcu32= (uint32 const *)addr_uo;
                addr_uo      = (rba_MemLib_UintPtr_tuo)curBfr2_pcu8;
                curBfr2_pcu32= (uint32 const *)addr_uo;
...
void rba_MemLib_MemSet_Provided(uint8 * dst_pu8, uint8 value_u8, uint32 length_u32)
{
    ...
    if(lenRemaining_u32 >= 4u)
    {
        ...
        uint32 value_u32 = (uint32)(0x01010101u) * value_u8;
        uint32 bytesUnalignedPre_u32;
        uint32 Loop_u32;
        uint32 bytesAligned_u32;
        ...
        /* alignment: 3 -> pre-copy 1 */
        /* Note: at least 4 bytes are to be copied because of condition above */
        bytesUnalignedPre_u32 = (4u - bytesUnalignedPre_u32) & 0x3u;
        ...
        {
            /* explicitely convert the pointers to 32bit as the alignment is 32bit aligned */
            /* MR12 RULE 11.4 VIOLATION: Pointer to/from integer is necessary as following access is performed with 32bit alignments. That was ensured before -> not critical as rba_MemLib_UintPtr_tuo is used as temporary integer */
            rba_MemLib_UintPtr_tuo addr_uo     = (rba_MemLib_UintPtr_tuo)curDst_pu8;
            destAddress_pu32                   = (uint32 *)addr_uo;
        }
        lenRemaining_u32    -= bytesUnalignedPre_u32;
        ...
        Loop_u32         = lenRemaining_u32 / 4u;
        bytesAligned_u32 = Loop_u32*4u;
    ...
    if(lenRemaining_u32 >= 4u)
    {
        ...
        if (flgRet)
        {
            ...
            curDst_pcu8          += bytesUnalignedPre_u32;
            {
                /* explicitely convert the pointers to 32bit as the alignment is 32bit aligned */
                /* MR12 RULE 11.4 VIOLATION: Cast of Pointers via integer is necessary as following access is performed with 32bit alignments. Alignment was ensured before. Conversion uses special integer type rba_MemLib_UintPtr_tuo */
                rba_MemLib_UintPtr_tuo addr_uo     = (rba_MemLib_UintPtr_tuo)curDst_pcu8;
                destAddress_pu32                   = (uint32 *)addr_uo;
            }
            lenRemaining_u32    -= bytesUnalignedPre_u32;

    ...
    for(loopCtr_u8 = 0; loopCtr_u8 < numChannels_u8 ;loopCtr_u8++)
    {
        ...
        if (PncState_en == COMM_PNC_REQUESTED)
        {
            ...
            #endif  /*  (COMM_PNC_GW_ENABLED != STD_OFF) && (COMM_ZERO_PNC_VECTOR_AVOIDANCE != STD_OFF)  */
            {
                ...
                /* ComM shall request COMM_FULL_COMMUNICATION_WITH_WAKEUP_REQUEST for all configured ComM channels for
                 * this PNC, If ComMWakeupSleepRequestEnabled is set to true and sub state COMM_PNC_REQUESTED
                 * is entered from COMM_PNC_NO_COM or COMM_PNC_PREPARE_SLEEP */
                if(((PncPreviousState_en == COMM_PNC_NO_COMMUNICATION) ||
                    (PncPreviousState_en == COMM_PNC_PREPARE_SLEEP)) &&
                    (channelConfigPtr_pcst->ComMWakeupSleepRequestEnabled_b == TRUE))
                {
                    /* Set ComMFullComWakeupRequestStatus_b flag to true for COMM_FULL_COMMUNICATION_WITH_WAKEUP_REQUEST
                     * request */
                    channelRamPtr_pst->ComMFullComWakeupRequestStatus_b = TRUE;
                }
                /* ComM shall request COMM_FULL_COMMUNICATION for all configured ComM channels for this PNC,
                 * if ComMWakeupSleepRequestEnabled is set to true and sub state COMM_PNC_REQUESTED is entered from
                 * COMM_PNC_READY_SLEEP or ComMWakeupSleepRequestEnabled is set to false or not available and sub state
                 * COMM_PNC_REQUESTED is entered from any other states.
                 * Increment PncRequest counter */
        ...
        else if((PncState_en == COMM_PNC_READY_SLEEP) && (PncPreviousState_en == COMM_PNC_REQUESTED))
        {
            ...
            #endif  /*  (COMM_PNC_GW_ENABLED != STD_OFF) && (COMM_ZERO_PNC_VECTOR_AVOIDANCE != STD_OFF)  */
            {
                ...
                else
                {
                    ...
                    COMM_DET_REPORT_ERROR(COMM_PNC_MAINFNC_APIID, COMM_E_CNTR_UNDERFLW);
                }
            }
        }
        else
...
void ComM_Prv_EIRA_CallBack(uint8 PnBusId_u8)
{
    ...
    if(COMM_GET_FEATURE_PNC_ENABLED != FALSE)
    {
        ...
        if(ComSigId_u16 != COMM_COMSIGNAL_INVALID_ID)
        {
            ...
            /* Initialise Pnc ram and config ptr variables */
            pncRamPtr_pst = &ComM_PncRamStruct[C_ZERO];
            pncConfigPtr_pcst = &(COMM_GET_PNC_LIST[C_ZERO]);
                ...
                if(pncConfigPtr_pcst->isPncGwCoordinated_b != FALSE)
                {
                    ...
                    if(ComM_ERA_RxSigVal[PnBusId_u8][PncIndex_u8] != ERA_PncSigValue_b)
                    {
                        ...
                        if(ERA_PncSigValue_b != FALSE)
                        {
                            ...
                            * ERACnt_u8 is not updated, state change will not happen and both these updated counters
                            * are considered only in the next Pnc main function. Thus avoiding locks.*/
                        ...
                        {
                            /* ActiveERACnt_u8 and ERACnt_u8 has to be updated and read consistently. But with the
                             * below order of implementation, locks are not required  in the Pnc main function */
                            /* If ActiveERACnt_u8 is updated before ERACnt_u8,then
                             * even though main function interrupts this function, when ActiveERACnt_u8 is updated but
                             * ERACnt_u8 is not updated, state change will not happen and both these updated counters
                             * are considered only in the next Pnc main function. Thus avoiding locks.*/
                                         ...
                                         boolean                    Status_b)
{
    ...
    #endif /*  (COMM_PNC_ENABLED != STD_OFF)  */
    /* For all users and PNC's of channel update inhibition counter depending on the inhibition type
    ...
    ComM_Prv_TranslateInhibitionToUser(channelConfigPtr_pcst->DirectUsers_pcu8,
            channelConfigPtr_pcst->numDirectUsers_u8,InhibitionType_en,Status_b);
    ...
    if(COMM_GET_FEATURE_PNC_ENABLED != FALSE)
    {
        for(numPnc_u8 = C_ZERO; numPnc_u8 < channelConfigPtr_pcst->numPncs_u8; ++numPnc_u8)
        {
            ...
            {
                ComM_Prv_UpdateInhibitionCounter(&(pncRamPtr_ps->WakeUpInhibitionCtr_u16),Status_b,
                        pncConfigPtr_pcs->NumChannels_u8);
            }
            else
            {
                ComM_Prv_UpdateInhibitionCounter(&(pncRamPtr_ps->LimitToNoComCtr_u16),Status_b,
                        pncConfigPtr_pcs->NumChannels_u8);
            }

            /* 3. For all users assigned to PNC of channel  */
            ComM_Prv_TranslateInhibitionToUser(pncConfigPtr_pcs->Users_pcu8,pncConfigPtr_pcs->NumUsers_u8,
                    InhibitionType_en,Status_b);
                                                      ...
                                                      boolean                   Status_b)
{
    ...
    for(loopCounter_u8 = C_ZERO; loopCounter_u8 < numUsers_u8; ++loopCounter_u8)
    {
        ...
        if(InhibitionType_en == COMM_PREVENTWAKEUP)
        {
            ComM_Prv_UpdateInhibitionCounter(&(userRamPtr_ps->WakeUpInhibitionCtr_u16),Status_b,
                    userConfigPtr_pcs->NumAllChannels_u8);
        }
        else
        {
            ComM_Prv_UpdateInhibitionCounter(&(userRamPtr_ps->LimitToNoComCtr_u16),Status_b,
                    userConfigPtr_pcs->NumAllChannels_u8);
        }
    }
 ...
 *                    Status -> Inhibition requested or released status
 *                    InhibitionCounterMax_u16 -> Max value of the Inhibition counter
 *                    that can be incremented for PNC/Channel
 * Return value     : void
 ****************************************************************************************************************/
                                                   ...
                                                   uint16    InhibitionCounterMax_u16)
{
    if(Status_b != FALSE)
    {
        ...
        {
            /* Control shouldn't come here.If at all happens, report to DET */
            COMM_DET_REPORT_ERROR(COMM_INHIBITION_COUNTER, COMM_E_IHHIBITION_CNTR_OVERFLW);
        }
        else
        {
    ...
    else
    {
        ...
        {
            /* Control shouldn't come here.If at all happens, report to DET */
            COMM_DET_REPORT_ERROR(COMM_INHIBITION_COUNTER, COMM_E_IHHIBITION_CNTR_UNDERFLW);
        }
        else
        {
            --(*InhibitionCounter_pu16);
...
LOCAL_INLINE boolean ComM_Prv_Getbit(uint8 PncId_u8, const uint8 * PnSignalVal_pu8)
{
    ...
    bitPosition_u8 = (PncId_u8 - (uint8)COMM_PNC_VECTOR_STARTBITPOSITION) % 8u;

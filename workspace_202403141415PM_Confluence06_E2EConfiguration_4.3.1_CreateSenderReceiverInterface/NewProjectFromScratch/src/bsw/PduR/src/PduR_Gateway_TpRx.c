...
/* MR12 RULE 8.13 VIOLATION: A pointer parameter in a function prototype should be declared as pointer to const
...
BufReq_ReturnType PduR_GF_Tp_StartOfReception_Func(PduIdType gwId,
        const PduInfoType* info,PduLengthType TpSduLength, PduLengthType* bufferSizePtr)
{
...

    {
        ...
        if(TpSduLength > fifo->connectionTable->secondarySession->bufferLength)
        {
            ...
            /* setting return value to BUFREQ_OK*/
            rtn = BUFREQ_OK;
#else
    ...
    if (SessionPtr->Rx_Status != PDUR_SESSION_IDLE)
    {
        ...
        {
            /* In case of FIFO enabled, check if there are any free instance where the new
             * reception can be accommodated */
            if(!((SessionPtr->Rx_Status == PDUR_RX_FINISHED) &&
...
#endif
    {
        PduR_Prv_UpdatePduR_Gw_Tp_ReadInfoStruct(gt,SessionPtr->bufferPtr);
        /* Update read ptr and write ptr to the buffer_pcst pointer */
    ...
    *bufferSizePtr = SessionPtr->bufferLength;
    ...
    if((info != NULL_PTR) && (info->MetaDataPtr != NULL_PTR) && (gt->MetaDataInfo_pst != NULL_PTR))
    {
        /* Copy the metadata into the buffer */
        rba_BswSrv_MemCopy8(SessionPtr->metaDataPtr,info->MetaDataPtr,gt->MetaDataInfo_pst->metaDataBufLen);
    }
...
BufReq_ReturnType PduR_GF_Tp_CopyRxData_Func( PduIdType gwId, const PduInfoType* info, PduLengthType* bufferSizePtr)
{
    ...
    if ((SessionPtr == NULL_PTR) || (SessionPtr->Rx_Status != PDUR_RX_INPROGRESS) ||
            (info->SduLength > (SessionPtr->SduLength_auo[SessionPtr->Rx_FifoInstanceUsed_u8] -
                    SessionPtr->numRxLength_auo[SessionPtr->Rx_FifoInstanceUsed_u8])))
    {
        return BUFREQ_E_NOT_OK;
    }
    ...
    if((SessionPtr->RxChunkLength == SessionPtr->bufferLength) &&
            (((SessionPtr->RxChunkLength - SessionPtr->TxChunkLength) != 0u) ||
                    (SessionPtr->numRxLength_auo[SessionPtr->Rx_FifoInstanceUsed_u8] == SessionPtr->numTxLength)))
    {
    ...
    else if((SessionPtr->numRxLength_auo[SessionPtr->Rx_FifoInstanceUsed_u8] == SessionPtr->numTxLength) ||
            (SessionPtr->RxChunkLength > SessionPtr->TxChunkLength))
    {
        ...
        {
            /* Total received length is equal to transmitted length or received chunk length is greater than
             * transmitted chunk length Copy the data into the receive buffer and update the write pointer */
            ...
            SessionPtr->RxChunkLength = 0;
            rba_BswSrv_MemCopy8(SessionPtr->writePtr,(info->SduDataPtr + availableBuffLength_l),
                    (info->SduLength - availableBuffLength_l));
        ...
        else
        {
            ...
            SessionPtr->RxChunkLength += info->SduLength;
            /* Update remaining buffer size info */
            SessionPtr->remainingBuffLength =
                    ((SessionPtr->bufferLength - SessionPtr->RxChunkLength) + SessionPtr->TxChunkLength);
    ...
    *(bufferSizePtr) = SessionPtr->bufferLength - SessionPtr->numRxLength_auo[SessionPtr->Rx_FifoInstanceUsed_u8];
    ...
    else
    {
        ...
        /* As MetaDataPtr is supported, MetaDataPtr is updated with a Valid address */
        readerPduInfo.MetaDataPtr = SessionPtr->metaDataPtr;
...
void PduR_GF_Tp_RxIndication_Func( PduIdType gwId, Std_ReturnType result)
{
    ...
    if (result == E_OK)
    {
        ...
        if(gt->TpQueueDepth_u8 > 1u)
        {
            ...
            SessionPtr->writePtr = gt->GW_Tp_buffer->begin;

            /* Check if FIFO is FULL or not. If not FULL, move the writePtr to next instance, else reset back the
             * pointer to begin of the buffer in TxConfirmation */
    ...
    {
#ifdef PDUR_FIFO_TP_SUPPORT
        /* Rx_FifoInstanceUsed_u8 and WritePtr is not required to move to next instance when
         * Tp_RxIndication result is E_NOT_OK */
        if(gt->TpQueueDepth_u8 > 1u)
        {
            /* Holds the length of the buffer to move the writeptr */
            PduLengthType RxInstanceLength_l;
            ...
            /* Reset the received data in CopyRxData() and in StartOfReception() of the same instance,
            ...
            {
                /* If RxIndication returns with E_NOT_OK with first reception, then to accept next reception,
                 * reset Rx_Status to IDLE */
                SessionPtr->Rx_Status = PDUR_SESSION_IDLE;
            }
            else
            {
                /* If RxIndication returns with E_NOT_OK with first reception, then to accept next reception,
                 * reset Rx_Status to FINISHED */
                SessionPtr->Rx_Status = PDUR_RX_FINISHED;
            }

            /* Transmission shall be aborted which was triggered from the CopyRxData().
             * Use Case: Two successful reception is done and two TxConfirmation is received,
             * assume during next reception's CopyRxData,
            ...
            if((SessionPtr->Rx_FifoInstanceUsed_u8 == SessionPtr->Tx_FifoInstanceUsed_u8) &&
                    (SessionPtr->Tx_Status == PDUR_TX_INPROGRESS))
            {
                SessionPtr->Tx_Status = PDUR_TX_IDLE;

...
/* HIS METRIC LEVEL VIOLATION IN CanSM_RequestComMode: Function contains very simple "else if" statements,
          ...
          (CanSM_TxToutException_state_en==CANSM_TxTimeoutException_DEFAULT)
         )
            {
               /* Transition to NO_COM */
                ...
                /*Store the no com request status*/
                CanSM_NoComRequested_ab[network] = TRUE;

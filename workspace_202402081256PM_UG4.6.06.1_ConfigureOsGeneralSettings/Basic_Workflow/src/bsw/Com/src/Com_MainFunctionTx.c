...
#if defined(COM_CANCELTRANSMITSUPPORT) && defined(COM_TX_TP_IPDUTYPE)
# include "PduR_Com.h"
...
void Com_Prv_InternalMainFunctionTx(Com_MainFunc_tuo idTxMainFunc_uo)
{
    ...
    * before the AUTOSAR COM module was initialized with Com_Init or after a call to Com_DeInit*/
    if (Com_InitStatus_en != COM_UNINIT)
    {
        ...
        Com_Prv_GwFifoToTx(idTxMainFunc_uo);
#endif

        /* Get the first TxIPdu-Id to be processed for given ComMainFunction */
        idxIdTxIpdu_qu16 = COM_GET_MAINFUNCTX_CFG(idTxMainFunc_uo)->idFirstIpdu_uo;
        numOfIpdus_qu16 = (idxIdTxIpdu_qu16 + COM_GET_MAINFUNCTX_CFG(idTxMainFunc_uo)->numOfIpdus_uo);
        ...
        for ( ; idxIdTxIpdu_qu16 < numOfIpdus_qu16; idxIdTxIpdu_qu16++ )
        {
            ...
            if (Com_Prv_CheckTxIPduStatus((PduIdType)idxIdTxIpdu_qu16))
            {
                ...
                if (Com_GetValue(TXIPDU,_ISGWPDU,txIpduConstPtr_pcst->txIPduFields_u16))
                {
                    ...
                    Com_Prv_PrepareGwTxIpdu((Com_IpduId_tuo)idxIdTxIpdu_qu16);
                }
                ...
                if (Com_GetRamValue(TXIPDU,_CONFIR,txIpduRamPtr_pst->txFlags_u16))
                {
                    ...
                    * hence notifications are invoked here */
                    notifyFlag_b = (Com_GetValue(TXIPDU,_SIGPROC,txIpduConstPtr_pcst->txIPduFields_u16) ==
                                                                                                        COM_DEFERRED);
...
#  else
                    Com_Prv_TxIPduToutCallbacks((Com_IpduId_tuo)idxIdTxIpdu_qu16);
...
#  else
                    Com_Prv_TxIPduAckCallbacks((Com_IpduId_tuo)idxIdTxIpdu_qu16);
#  endif /* end of COM_CONFIGURATION_VARIANT */
                }
                else
#endif /* #ifdef COM_TxIPduNotification */
       ...
       (Com_GetRamValue(TXIPDU,_TICKTXTO,txIpduRamPtr_pst->txFlags_u16) == COM_START))
    {
        ...
        if (txIpduRamPtr_pst->cntrTxTimeout_u16 == COM_ZERO)
        {
            ...
            * if the transmission deadline monitoring timer expires */
            ...
            isCancelTransmitSupported_b = Com_GetValue(TXIPDU,_ISCANCELTRANSMITSUPPORTED,
                                                                               txIpduConstPtr_pcst->txIPduFields_u16);
...
#endif /* #if defined(COM_CANCELTRANSMITSUPPORT) && defined(COM_TX_TP_IPDUTYPE) */
        }
    }
...
LOCAL_INLINE boolean Com_Prv_MainFunctionTx_Modes(uint16_least idTxPdu_qu16)
{
    ...
    txIpduRamPtr_pst = COM_GET_TX_IPDU_CONSTDATA(idTxPdu_qu16)->ipduRam_pst;
 ...
 **********************************************************************************************************************
 ...
 Description      : Service to perform the Direct Mode transmision processing of the AUTOSAR COM
                    transmission activities that are not directly initiated by the calls from the RTE and PDU-R.
 Parameter        : void
 Return value     : None
 **********************************************************************************************************************
*/
...
#elif (defined (COM_SIGNALGATEWAY) || defined (COM_SIGNALGROUPGATEWAY))
            /* MR12 RULE 13.5 VIOLATION: Below conditional statement access RAM, use of
            * intermediate variables to store and read here, may read obsolete value.
            * Evaluation(or no evaluation) shall not impact the system behaviour.
            * Hence suppressed. */
            if ((txIpduRamPtr_pst->cntrRepPeriod_u16 == COM_ZERO) &&
               (Com_GetRamValue(TXIPDU,_GWIPDU_TXREQ,txIpduRamPtr_pst->txFlags_u16)))
            {

                /* Tx Gw IPDU direct frames of MIXED mode */
                sendIPdu_b = COM_TRUE;
            }
            else
...
LOCAL_INLINE boolean Com_Prv_MainFunctionTx_MixedMode(uint16_least idTxPdu_qu16)
{
    ...
    if (txIpduRamPtr_pst->cntrTimePeriod_u16 == COM_ZERO)
    {
        ...
        txIpduRamPtr_pst->cntrTimePeriod_u16 = COM_GET_TXIPDU_TIMEPERIOD(idTxPdu_qu16,
...
LOCAL_INLINE void Com_Prv_MainFunctionTx_SendIPdu(uint16_least idTxPdu_qu16)
{
    ...
    * In case of Normal pdu: Then this flag will never be set, so always Com_Prv_SendIpdu() will be called */
    ...
    if (COM_TRUE ==
        Com_GetRamValue(TXIPDU,_LARGEDATAINPROG,COM_GET_TX_IPDU_CONSTDATA(idTxPdu_qu16)->ipduRam_pst->txFlags_u16))
    {

        COM_DET_REPORT_RUNTIME_ERROR(COMServiceId_MainFunctionTx, COM_E_SKIPPED_TRANSMISSION)
...
#endif /* end of COM_PRV_ERROR_HANDLING */
    {
        ...
        * the configured Pdu is Periodic and it is scheduled to be Transmitted in the coming cycle */
        if (Com_GetRamValue(TXIPDU,_LATESTMODE,txIpduRamPtr_pst->transMode_u8) == COM_TXMODE_PERIODIC)
        {
            ...
            timeBaseInMs_uo = COM_GET_MAINFUNCTX_CFG(txIpduConstPtr_pcst->idMainFunc_uo)->timeBaseInMs_uo;

            /* In normal scenario, it is not intended that caller of Com_IsTxScheduled is called in lesser task cycle
            * than MainFunctionTx. Check if this is the case. */
            ...
            else
            {
                ...
                * the remaining time from un-decremented ticks, then time-value will be more than the actual remaining
                * time. Hence remaining time is computed from the decremented tick counter locally,

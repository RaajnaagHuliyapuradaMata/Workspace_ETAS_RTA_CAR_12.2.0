...
Std_ReturnType CanIf_XCore_PipeWrite(uint16 PipeId, CanIf_XCore_PipeTxRxParams_st * params_pst)
{
    ...
    uint32 localCoreIrqState_tu32 = 0u;
    ...
    SCHM_CANIF_NONEST_DISABLE_CORE_LOCAL_IRQ(&localCoreIrqState_tu32);
	...
	if(retVal == E_OK)
	{
		...
		if (((uint32)Canif_XCore_info_pcst->SduDataPtr & 0x3u) == 0x0u)
		{
	...
	/*close the ctitical section to allow data copy (long) to not block the CPU IRQs */
	SCHM_CANIF_NONEST_RESTORE_CORE_LOCAL_IRQ(localCoreIrqState_tu32);

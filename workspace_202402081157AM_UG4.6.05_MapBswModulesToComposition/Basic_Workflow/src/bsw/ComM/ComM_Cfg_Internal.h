...
#define COMM_SYNCHRONIZED_PNC_SHUTDOWN      STD_OFF
...
/* Number of possible Bus types which have EIRA configured.
   The Const and RAM structures to store EIRA information is based on the ComMChannels configured rather than the 
   actual EIRA Signals present. This is done to keep the implementation common for both pre-compile and post-build
   variants.
*/
...
typedef struct
{
    ...
    boolean                         ComMWakeupSleepRequestEnabled_b;
...
typedef struct
{
    ...
    ComM_PncCoordinatorType_ten PncCoordinatorType_en;  /* Holds the Type of PNC Co-ordinator */
#endif
...
#endif
    boolean             ComMPncWakeupSleepRequestEnabled_b;

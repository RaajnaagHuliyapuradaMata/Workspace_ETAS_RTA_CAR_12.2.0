...
/* Syntax           : Std_ReturnType CanIf_Transmit(    PduIdType CanIfTxSduId,
        ...
        const PduInfoType * CanIfTxInfoPtr)
{
    ...
    CANIF_DET_REPORT_ERROR_INIT_NOT_OK((FALSE == CanIf_Prv_InitStatus_b), CANIF_TRANSMIT_SID, CANIF_E_UNINIT)
    ...
    if((lTxPduConfig_pst->TxTruncEnabled_b == FALSE) && (CanIfTxInfoPtr->SduLength> (uint32)lTxPduConfig_pst->TxPduLength_u8))
    {
         ...
         * truncation is disabled */
          CANIF_DET_RUN_TIME_REPORT_ERROR_NOT_OK(TRUE, CANIF_TRANSMIT_SID, CANIF_E_TXPDU_LENGTH_EXCEEDED)
...
/* HIS METRIC v(G),STMT,LEVEL VIOLATION IN CanIf_XCore_LocalCore_Transmit: Function contains very simple "if-else" statements.
                                               ...
                                               )
{
    ...
    /* TxConfirmation Notification return value */
    Std_ReturnType ULConfirmationRetVal_en = E_OK;
    #endif
    /* Pointer to Tx Pdu configuration */
    const CanIf_Cfg_TxPduConfig_tst * lTxPduConfig_pst;
    ...
    Std_ReturnType lCanStatus_en        = E_NOT_OK;
...
/* If PduInfoPtr->MetaDataPtr is null when metadata lenth is configured , report to DET and return E_NOT_OK*/
#if(CANIF_METADATA_SUPPORT == STD_ON)
    CANIF_DET_REPORT_ERROR_NOT_OK(((NULL_PTR == CanIfTxInfoPtr->MetaDataPtr)&&(lTxPduConfig_pst->MetaDataLength >= 1)), CANIF_TRANSMIT_SID, CANIF_E_PARAM_POINTER)
#endif
    ...
    CANIF_DET_RUN_TIME_REPORT_ERROR_NOT_OK((Pdu_mode_Lcl == (uint8)CANIF_OFFLINE), CANIF_TRANSMIT_SID, CANIF_E_STOPPED)
/*[SWS_CANIF_00677] Return E_NOT_OK if Controller mode is CAN_CS_STOPPED */
if(Ctrl_mode_Lcl==(uint8)CAN_CS_STOPPED)
{
    lRetVal_en = E_NOT_OK;
}
else
    ...
    /*[SWS_CANIF_00750] */
    ...
    #endif
       {
           if(
                           ...
                           (( Ctrl_mode_Lcl == (uint8)CAN_CS_STARTED) && (Pdu_mode_Lcl == (uint8)CANIF_ONLINE))
           #if (CANIF_PUBLIC_PN_SUPPORT == STD_ON)
                                   )
           #endif
                   ...
                   /* MR12 RULE 12.1 VIOLATION: Warning is "Not Critical".Verified through testing that rules of precedence and associativity are not affecting the functionality   */
                           ||((Ctrl_mode_Lcl == (uint8)CAN_CS_STARTED) && (((Pdu_mode_Lcl  == (uint8)CANIF_TX_TP_ONLINE) && \
...

               {
           ...
           #if(CANIF_METADATA_SUPPORT == STD_ON)
                       if((lTxPduConfig_pst->MetaDataLength >= 1) &&(CanIfTxInfoPtr->MetaDataPtr!=NULL_PTR))
                       {
                           if(CanIfTxInfoPtr->SduLength>(uint32)lTxPduConfig_pst->TxPduLength_u8)
                           {
                               lSduLength = lTxPduConfig_pst->TxPduLength_u8;
                           }
                           else
                          ...
                          /* As per CanId type, set the maximum sdulength possible*/
                           ...
                           /*pointer to metadataPtr containing metadata*/
                           lMetaDataPtr_u8 = (uint8 *)(CanIfTxInfoPtr->MetaDataPtr);
                           ...
                           */
                           ...
                           lMetaDataPtr_u8 = lMetaDataPtr_u8 + (lTxPduConfig_pst->MetaDataLength-1u);
                           ...
                           else                   /*When no CanIfTxPduCanId is configured*/
                           {
                               ...
                               /*MetaData shall be used directly as CanId (only if MetaDataItemLength>= CanID size)*/
                               /*Note: if CanIfTxPduCanId is not configured MetaDataItemLength is always ensured to be
                               large enough as actual CanId size. This is done in validations*/

                               /*As per [SWS_CANIF_00846], If MetaDataItemLength is larger than the CanId size, the space after
                   ...
                   CANIF_DET_RUN_TIME_REPORT_ERROR(((uint8)CanIfTxInfoPtr->SduLength > lMaxSduLength_u8), \
           CANIF_TRANSMIT_SID, CANIF_E_DATA_LENGTH_MISMATCH)
               ...
               L-PDU Buffer (CanIfTxBuffer), if it is rejected by CanDrv at Transmit Request.*/
                       ...
                       if((lTxPduConfig_pst->CanIf_TxBufferConfigPtr->CanIfBufferSize) > 0)
                       {
                           ...
                           CANIF_DET_RUN_TIME_REPORT_ERROR(((lRetVal_en == E_OK) && ((uint8)lPduInfo_st.length > lDataMaxLen)),     \
                                                                               CANIF_TRANSMIT_SID, CANIF_E_DATA_LENGTH_MISMATCH)
                       }
                       else
                       {
           ...
           #if(CANIF_TXOFFLINEACTIVE_SUPPORT == STD_ON)
               else if(((uint8)CAN_CS_STARTED == Ctrl_mode_Lcl )&&
                           (Pdu_mode_Lcl == (uint8)CANIF_TX_OFFLINE_ACTIVE))
               {
                   if(NULL_PTR != lTxPduConfig_pst->UserTxConfirmation)
                   {
                       lTxPduConfig_pst->UserTxConfirmation(lTxPduConfig_pst->TxPduTargetPduId, ULConfirmationRetVal_en);
                   }

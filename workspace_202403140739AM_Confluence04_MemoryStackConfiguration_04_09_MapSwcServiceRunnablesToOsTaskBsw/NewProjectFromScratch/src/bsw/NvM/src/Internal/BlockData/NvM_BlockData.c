 ...
 *   TRACE[BSW_SWCS_AR_NVRAMManager_Ext-2978]
 *
 ...
 *
 * - bit 8 = state of the job currently ongoing for this block (bit set if a job is ongoing)
 *           (access mask = NVM_BLOCK_STATE_JOB_ONGOING)
 */
uint16 NvM_Prv_stBlock_auo[NVM_CFG_NR_ALL_BLOCKS];
...
uint32 NvM_Prv_cntrWrite_auo[NVM_CFG_NR_ALL_BLOCKS];
#endif
 ...
 * TRACE[NVM134]
 */
 // If the NvM_Prv_stRequests_au16 is changed here, keep the measurement definition of it in NvMArxmlGen.java in sync
 ...
 * - NVM_REQ_OK                (0u) = Last asynchronous request finished successfully
 * - NVM_REQ_NOT_OK            (1u) = Last asynchronous request finished unsuccessfully
 * - NVM_REQ_PENDING           (2u) = An asynchronous request is currently pending
 * - NVM_REQ_INTEGRITY_FAILED  (3u) = Data integrity failure for last read operation
 * - NVM_REQ_BLOCK_SKIPPED     (4u) = Referenced block skipped in NvM_ReadAll or NvM_WriteAll
 * - NVM_REQ_NV_INVALIDATED    (5u) = Referenced block invalidated
 * - NVM_REQ_CANCELED          (6u) = Multiblock write request canceled
 * - NVM_REQ_REDUNDANCY_FAILED (7u) = Required redundancy of the referenced block lost
 * - NVM_REQ_RESTORED_FROM_ROM (8u) = Referenced block restored from ROM
 *
 * TRACE[NVM134]
 * TRACE[NVM136]
 */
// If the NvM_Prv_stRequestResult_au8 is changed here, keep the measurement definition of it in NvMArxmlGen.java in sync
NvM_RequestResultType NvM_Prv_stRequestResult_au8[NVM_PRV_NR_BLOCKS_ADMIN];

#if (NVM_RB_ERROR_INFO_DETAIL == STD_ON)
/**
 * Global private array with detailed error information for each NVRAM-block.
 *
 * Request results are part of administrative-block as it is required by AUTOSAR.
 * See defined error details in definition of NvM_Rb_ErrorInfoDetail_ten
 */
// If the NvM_Prv_stDetailedErrorInfo_aen is changed here, keep the measurement definition of it in NvMArxmlGen.java in sync
NvM_Rb_ErrorInfoDetail_ten NvM_Prv_stDetailedErrorInfo_aen[NVM_PRV_NR_BLOCKS_ADMIN];
#endif
 ...
 * In this case NvM resets the write counter and if block is configured for block compression then NvM also resets
 * the corresponding master-block.
 ...
 * In this case NvM does not initialize the write counter and the corresponding master-block
 * because there are 2 possibilities:
 * - if RAM-block is still consistent then the write counter and the corresponding master-block is consistent too
 *   -> no initialization required
 * - if RAM-block is not consistent then the NVRAM block will participate in NvM_ReadAll and this will
 *   initialize the write counter and read master-block from the non-volatile memory.
 *
 * This function resets always the ongoing job status, especially if there was a soft reset, to make sure
 * that no internal job is pending after initialization.
 *
 ...
 */
void NvM_Prv_Block_InitializeData(boolean const *isSavedZoneDataLost_pcb)
{
    NvM_BlockIdType idBlock_uo;

    // Initialize all administrative blocks
    for (idBlock_uo = NVM_PRV_MULTI_BLOCK; idBlock_uo < NVM_CFG_NR_ALL_BLOCKS; ++idBlock_uo)
    {
        boolean isRamBlockCrcEnabled_b = NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo, NVM_PRV_BLOCK_FLAG_USE_RAM_CRC);
        boolean isUnconditionalInitRequired_b = NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo,
                                                                                NVM_PRV_BLOCK_FLAG_RAM_INIT_UNCONDITIONAL);
        boolean isSavedZoneDataLost_b = isSavedZoneDataLost_pcb[NvM_Prv_BlkDesc_GetIdxSavedZone(idBlock_uo)];
        // Always reset status of ongoing job
        NvM_Prv_stBlock_auo[idBlock_uo] &= ~(NVM_BLOCK_STATE_JOB_ONGOING);

        // Always reset "RAM CRC Ongoing" and "RAM CRC Recalc Req". For blocks without
        // RAM Block CRC configured this doesn't harm.
        NvM_Prv_stBlock_auo[idBlock_uo] &= ~(  NVM_BLOCK_STATE_RAM_CRC_RECALC_ONGOING
        ...
        {
            NvM_Prv_stBlock_auo[idBlock_uo] = 0u;
#if (defined(NVM_RB_BLOCK_WRITE_COUNTER) && (STD_ON == NVM_RB_BLOCK_WRITE_COUNTER))
            NvM_Prv_cntrWrite_auo[idBlock_uo] = 0u;
#endif
            // initialize master-blocks for all blocks configured for block splitting
            NvM_Prv_BlockSplit_Initialize(idBlock_uo);
        }
        ...
        if (isUnconditionalInitRequired_b || isSavedZoneDataLost_b)
        {
            ...
            if (NvM_Prv_BlkDesc_IsBlockSelected(idBlock_uo, NVM_PRV_BLOCK_FLAG_WRITE_PROTECTED))
            {
                NvM_Prv_stBlock_auo[idBlock_uo] |= NVM_BLOCK_STATE_WRITE_PROT;
            }
        }
...
#if (NVM_RB_ERROR_INFO_DETAIL == STD_ON)
        // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3403] The default value after reset is NvM_Rb_ErrorInfoDetail_NotAvailable_e
        NvM_Prv_stDetailedErrorInfo_aen[idBlock_uo] = NvM_Rb_ErrorInfoDetail_NotAvailable_e;
#endif
...
#if (NVM_RB_ERROR_INFO_DETAIL == STD_ON)
    // TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3403] The default value after reset is NvM_Rb_ErrorInfoDetail_NotAvailable_e
    NvM_Prv_stDetailedErrorInfo_aen[NVM_PRV_AUX_ADMIN_BLOCK] = NvM_Rb_ErrorInfoDetail_NotAvailable_e;
#endif
    // TRACE[NVM192] The dataset index is initialized to zero during init
...
/**
 * This function checks whether the NvM is enqueuing the requested block for a multi-block service and
 ...
 * If called with enabled scheduler lock the run-time of 2 can be used (12 in worst case, but this is rather improbable):
 * - 1 for function call
 * - 1 for internal copy-operation
 * - NvM_Prv_Block_IsNvmEnqueuingForMulti_pfct = normally this is a null pointer, i.e. run-time is 0u;
 ...
 *                                               NvM_Prv_WriteAll_IsBlockParticipating, run-time of this function is 10
 *
 * \param[in] idBlock_uo
                                             ...
                                             NvM_Prv_Service_InitiationErrors_tuo* Errors_puo)
{
    ...
    {
        IsBlockPartOfMulti_b = NvM_Prv_Block_IsNvmEnqueuingForMulti_pfct(idBlock_uo, Errors_puo);
    }
 ...
 *
 * TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3169]
 */
NvM_BlockManagementType NvM_Rb_GetBlockType(NvM_BlockIdType BlockId)
{
    ...
    {
        BlockType_uo = NvM_Prv_BlkDesc_GetType(BlockId);
    }
 ...
 *
 * TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3167]
 */
 ...
 *
 * TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3164]
 */
boolean NvM_Rb_IsBlockSelectedForInitAtLayoutChange(NvM_BlockIdType BlockId)
{
    ...
    if ((BlockId >= NVM_CFG_FIRST_USED_BLOCK) && (BlockId < NVM_CFG_NR_BLOCKS))
    {
        isBlockSelected_b = NvM_Prv_BlkDesc_IsBlockSelected(BlockId, NVM_PRV_BLOCK_FLAG_SELECT_FOR_INIT_AT_LAYOUT_CHANGE);
    }
 ...
 *
 * TRACE[BSW_SWCS_AR_NVRAMManager_Ext-3162]
 */
boolean NvM_Rb_IsBlockSelectedForFirstInitAll(NvM_BlockIdType BlockId)
{
    ...
    if ((BlockId >= NVM_CFG_FIRST_USED_BLOCK) && (BlockId < NVM_CFG_NR_BLOCKS))
    {
        isBlockSelected_b = NvM_Prv_BlkDesc_IsBlockSelected(BlockId, NVM_PRV_BLOCK_FLAG_SELECT_FOR_FIRST_INIT_ALL);
    }
    return isBlockSelected_b;
}
